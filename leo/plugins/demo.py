...
#@+<< docstring >>
#@+node:ekr.20170128213103.2: ** << docstring >>
#@@language rest
...
#@-<< docstring >>
#@+<< imports >>
#@+node:ekr.20170128213103.3: ** << imports >>
import random
...
#@@tabwidth -4
#@+others
#@+node:ekr.20170128213103.4: ** top-level
...
class ScreenCastController(object):
    ...
    #@+node:ekr.20170128213103.10: *3* sc.Entry points
    #@+node:ekr.20170128213103.11: *4* sc.body_keys
    def body_keys(self, s, n1=None, n2=None):
        ...
        for ch in s:
            ...
            m.wait(n1, n2)
        c.redraw()
    #@+node:ekr.20170128213103.12: *4* sc.caption and abbreviations: body, log, tree
    ...
    #@+node:ekr.20170128213103.13: *4* sc.clear_log
    def clear_log(self):
        '''Clear the log.'''
        m = self
        m.c.frame.log.clearTab('Log')
    #@+node:ekr.20170128213103.14: *4* sc.command
    def command(self, command_name):
        ...
        c.redraw_now()
        m.repaint('all')
    #@+node:ekr.20170128213103.15: *4* sc.dismiss_menu_bar
    def dismiss_menu_bar(self):
        ...
        menubar.repaint()
    #@+node:ekr.20170128213103.16: *4* sc.find_screencast & helpers
    ...
    #@+node:ekr.20170128213103.17: *5* sc.find_next_screencast
    def find_next_screencast(self, p):
        ...
        while p:
            if p.h.startswith('@screencast'):
                ...
                p.moveToThreadNext()
        return None
    #@+node:ekr.20170128213103.18: *5* sc.find_prev_screencast
    def find_prev_screencast(self, p):
        ...
        while p:
            if p.h.startswith('@screencast'):
                ...
                p.moveToThreadBack()
        return None
    #@+node:ekr.20170128213103.19: *4* sc.focus
    def focus(self, pane):
        ...
        if f:
            ...
            g.trace('bad pane: %s' % (pane))
    #@+node:ekr.20170128213103.20: *4* sc.head_keys
    def head_keys(self, s, n1=None, n2=None):
        ...
        p.h = s
        c.redraw()
    #@+node:ekr.20170128213103.21: *4* sc.image
    def image(self, pane, fn, center=None, height=None, width=None):
        ...
        if parent:
            ...
            g.trace('bad pane: %s' % (pane))
            return None
    #@+node:ekr.20170128213103.22: *4* sc.open_menu
    def open_menu(self, menu_name):
        ...
        return menu
    #@+node:ekr.20170128213103.23: *4* sc.plain_keys
    ...
    #@+node:ekr.20170128213103.24: *4* sc.quit
    def quit(self):
        ...
        m.quit_flag = True
        c.bodyWantsFocus()
    #@+node:ekr.20170128213103.25: *4* sc.redraw
    def redraw(self, p=None):
        m = self
        m.c.redraw_now(p)
    #@+node:ekr.20170128213103.26: *4* sc.repaint
    def repaint(self, pane):
        ...
        if w:
            ...
            g.trace('bad pane: %s' % (pane))
    #@+node:ekr.20170128213103.27: *4* sc.select_position
    def select_position(self, p):
        m = self
        assert p
        m.redraw(p)
    #@+node:ekr.20170128213103.28: *4* sc.single_key
    ...
    #@+node:ekr.20170128213103.29: *3* sc.State handling
    #@+node:ekr.20170128213103.30: *4* sc.next & helper
    def next(self):
        ...
        while m.p:
            ...
            m.quit()
    #@+node:ekr.20170128213103.31: *5* sc.exec_node
    def exec_node(self, p):
        ...
        try:
            ...
            g.es_exception()
            m.quit()
    #@+node:ekr.20170128213103.32: *4* sc.prev
    def prev(self):
        ...
        if m.p and m.p == m.p1:
            ...
            if p and p == m.p1:
                ...
                m.start(m.p1)
    #@+node:ekr.20170128213103.33: *4* sc.start
    def start(self, p):
        ...
        m.state_handler()
    #@+node:ekr.20170128213103.34: *4* sc.start_commands (new)
    def start_commands(self, commands):
        ...
        m.delete_widgets()
            # Clear widgets left over from previous, unfinished, slideshows.
        m.state_handler()
    #@+node:ekr.20170128213103.35: *4* sc.state_handler
    ...
    #@+node:ekr.20170128213103.36: *4* sc.undo
    def undo(self):
        ...
        if m.node_stack:
            ...
            return None
    #@+node:ekr.20170128213103.37: *4* sc.set_state & clear_state
    ...
    #@+node:ekr.20170128213103.38: *3* sc.Utilities
    #@+node:ekr.20170128213103.39: *4* sc.get_key_event
    def get_key_event(self, ch, w):
        ...
        return leoGui.LeoKeyEvent(c, key, stroke,
            ...
            x=0, y=0,
            x_root=0, y_root=0)
    #@+node:ekr.20170128213103.40: *4* sc.delete_widgets
    def delete_widgets(self):
        m = self
        for w in m.widgets:
            w.deleteLater()
        m.widgets = []
    #@+node:ekr.20170128213103.41: *4* sc.pane_widget
    def pane_widget(self, pane):
        ...
        return d.get(pane)
    #@+node:ekr.20170128213103.42: *4* sc.resolve_icon_fn
    def resolve_icon_fn(self, fn):
        ...
        if g.os_path_exists(path):
            ...
            return None
    #@+node:ekr.20170128213103.43: *4* sc.wait
