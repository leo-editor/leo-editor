...
'''
#@+<< docstring >>
#@+node:ekr.20160331124028.2: ** << docstring >> (VR3)
#@@language rest
#@@wrap

viewrendered3.py.

Creates a window for *live* rendering of reSTructuredText, markdown text, images, movies, sounds, rst, html, etc.
...
Terry Brown created viewrendered.py, the free_layout and NestedSplitter plugins used by viewrendered.

Jacob Peck added markdown support.

Peter Mills created viewrendered2, based on the viewrendered.py plugin.

Edward K. Ream created viewrendered3.py and added communication and coordination between the free_layout, NestedSplitter and viewrendered plugins.
#@-<< docstring >>
'''
#@+<< to do >>
#@+node:ekr.20160331123847.3: ** << to do >> (VR3)
#@+at
# To do:
# 
...
#@+<< imports >>
#@+node:ekr.20160331123847.4: ** << imports >> (VR3)
...
#@-<< imports >>
#@+<< define stylesheet >>
#@+node:ekr.20160331123847.5: ** << define stylesheet >> (VR3)
...
#@+others
#@+node:ekr.20160331124028.3: ** imports (VR2)
import leo.core.leoGlobals as g
import leo.plugins.qt_text as qt_text
...
def show_scrolled_message(tag, kw):
    ...
    return True
#@+node:ekr.20160331123847.11: ** Commands
...
#@+node:ekr.20160331123847.23: ** class ViewRenderedProvider
class ViewRenderedProvider:
    #@+others
    #@+node:ekr.20160331123847.24: *3* __init__
    def __init__(self, c):
        self.c = c
        # Careful: we may be unit testing.
        if hasattr(c, 'free_layout'):
    ...
    #@+node:ekr.20160331123847.25: *3* ns_provides
    def ns_provides(self):
        return [('Viewrendered', '_leo_viewrendered')]
    #@+node:ekr.20160331123847.26: *3* ns_provide
    def ns_provide(self, id_):
        ...
        if id_ == '_leo_viewrendered':
            ...
            return vr
    #@-others
#@+node:ekr.20160331123847.27: ** class ViewRenderedController (QWidget)
if QtWidgets:
    ...
    class ViewRenderedController(QtWidgets.QWidget):
        ...
        def underline(self, s):
            ...
            # return '%s\n%s\n%s\n\n' % (ch*n,s,ch*n)
            return '%s\n%s\n\n' % (s, ch * n)
        #@+node:ekr.20160331123847.36: *3* update & helpers
...
if 0:
    ...
    def decorate_window(w):
        ...
        w.setWindowIcon(QtGui.QIcon(g.app.leoDir + "/Icons/leoapp32.png"))
        w.resize(600, 300)
    #@+node:ekr.20160331124028.7: *4* init
    def init():
        '''Return True if the plugin has loaded successfully.'''
        g.plugin_signon(__name__)
        g.registerHandler('after-create-leo-frame', onCreate)
        g.registerHandler('scrolledMessage', show_scrolled_message)
        return True
    #@+node:ekr.20160331124028.8: *4* onCreate
    def onCreate(tag, keys):
        c = keys.get('c')
        if c:
            ViewRenderedProvider(c)
        return
    #@+node:ekr.20160331124028.9: *4* show_scrolled_message
    def show_scrolled_message(tag, kw):
        if g.unitTesting:
            return # This just slows the unit tests.
        c = kw.get('c')
        ...
        return True
    #@+node:ekr.20160331124028.10: *3* Commands
    #@+node:ekr.20160331124028.11: *4* g.command('preview')
    @g.command('preview')
    def preview(event):
        '''A synonym for the vr-toggle command.'''
        toggle_rendering_pane(event)
    #@+node:ekr.20160331124028.12: *4* g.command('vr')
    @g.command('vr')
    def viewrendered(event):
        """Open render view for commander"""
        ...
        if vr:
            ...
            if hasattr(c, 'free_layout'):
                ...
                vr.show()
        return vr
    #@+node:ekr.20160331124028.13: *4* g.command('vr-contract')
    @g.command('vr-contract')
    def contract_rendering_pane(event):
        '''Expand the rendering pane.'''
        c = event.get('c')
        if c:
            ...
            if vr:
                ...
                # Just open the pane.
                viewrendered(event)
    #@+node:ekr.20160331124028.14: *4* g.command('vr-expand')
    @g.command('vr-expand')
    def expand_rendering_pane(event):
        '''Expand the rendering pane.'''
        c = event.get('c')
        if c:
            ...
            if vr:
                vr.expand()
    #@+node:ekr.20160331124028.15: *4* g.command('vr-hide')
    @g.command('vr-hide')
    def hide_rendering_pane(event):
        '''Close the rendering pane.'''
        global controllers
    ...
    close_rendering_pane = hide_rendering_pane
    #@+node:ekr.20160331124028.16: *4* g.command('vr-lock')
    @g.command('vr-lock')
    def lock_rendering_pane(event):
        '''Pause or play a movie in the rendering pane.'''
        c = event.get('c')
        if c:
            ...
            if vr and not vr.locked:
                vr.lock()
    #@+node:ekr.20160331124028.17: *4* g.command('vr-pause-play')
        ...
        if c:
            ...
            if vr and vr.vp:
                ...
                if vp.isPlaying():
                    ...
                    vp.play()
    #@+node:ekr.20160331124028.18: *4* g.command('vr-show')
    @g.command('vr-show')
    def show_rendering_pane(event):
        '''Show the rendering pane.'''
        c = event.get('c')
        if c:
            ...
            if vr:
                ...
                viewrendered(event)
    #@+node:ekr.20160331124028.19: *4* g.command('vr-toggle')
    @g.command('vr-toggle')
    def toggle_rendering_pane(event):
        '''Toggle the rendering pane.'''
        c = event.get('c')
        if c:
            ...
            if vr:
                ...
                viewrendered(event)
    #@+node:ekr.20160331124028.20: *4* g.command('vr-unlock')
    @g.command('vr-unlock')
    def unlock_rendering_pane(event):
        '''Pause or play a movie in the rendering pane.'''
        c = event.get('c')
        if c:
            ...
            if vr and vr.locked:
                vr.unlock()
    #@+node:ekr.20160331124028.21: *4* g.command('vr-update')
    @g.command('vr-update')
    def update_rendering_pane(event):
        '''Update the rendering pane'''
        c = event.get('c')
        if c:
            ...
            if vr:
                vr.update(tag='view', keywords={'c': c, 'force': True})
    #@+node:ekr.20160331124028.22: *3* class WebViewPlus (QWidget)
    class WebViewPlus(QtWidgets.QWidget):
        #@+others
        #@+node:ekr.20160331124028.23: *4* ctor (WebViewPlus) & helpers
        ...
        #@+node:ekr.20160331124028.24: *5* init_view
        def init_view(self):
            '''Init the vr pane.'''
            ...
            return view
        #@+node:ekr.20160331124028.25: *5* init_config
        def init_config(self):
            ...
            return ds
        #@+node:ekr.20160331124028.26: *5* init_timer
        def init_timer(self):
            ...
            return timer
        #@+node:ekr.20160331124028.27: *5* get_mode
        def get_mode(self):
            ...
            return default
        #@+node:ekr.20160331124028.28: *5* tooltip_text
        def tooltip_text(self, s):
            ...
            return s
        #@+node:ekr.20160331124028.29: *4* gui helpers
        #@+node:ekr.20160331124028.30: *5* getUIconfig
        ...
        #@+node:ekr.20160331124028.31: *5* lock
        def lock(self):
        ...
        #@+node:ekr.20160331124028.32: *5* render_rst
        def render_rst(self, s, keywords):
            ...
            if show_scrolled_message and got_docutils:
                ...
                self.timer.start()
        #@+node:ekr.20160331124028.33: *5* render_md
        def render_md(self, s, keywords):
            ...
            if self.auto:
                self.timer.start()
        #@+node:ekr.20160331124028.34: *5* render_html
        def render_html(self, html, keywords = None):
            ...
            if self.auto:
                self.timer.start()
        #@+node:ekr.20160331124028.35: *5* restore_scroll_position
        ...
        #@+node:ekr.20160331124028.36: *5* setHtml (EKR)
        def setHtml(self, s):
            
            self.view.setHtml(s)
        #@+node:ekr.20160331124028.37: *5* state_change
        def state_change(self, checked):
            """A wrapper for 'render' to re-render on all QAction state changes."""
            self.render_delegate()
        #@+node:ekr.20160331124028.38: *4* render_delegate
        def render_delegate(self):
            ...
            if mode == 'md':
                ...
                self.render()
        #@+node:ekr.20160331124028.39: *4* render & helpers
        def render(self):
            ...
            if self.rendering:
                ...
                try:
                    ...
                    self.rendering = False
        #@+node:ekr.20160331124028.40: *5* render_helper & helper
        def render_helper(self):
            ...
            if not self.auto:
                ...
                self.app.processEvents()
                self.pbar_action.setVisible(False)
        #@+node:ekr.20160331124028.41: *6* to_html & helper
        def to_html(self, p):
            ...
            try:
                ...
                if 'SEVERE' in msg or 'FATAL' in msg:
                    ...
                    return html
        #@+node:ekr.20160331124028.42: *7* process_nodes & helpers
        def process_nodes(self, p, tree=True):
            ...
            return s
        #@+node:ekr.20160331124028.43: *8* code_directive
        def code_directive(self, lang):
            ...
            if pygments:
                ...
                return '\n\n::\n\n'
        #@+node:ekr.20160331124028.44: *8* initCodeBlockString (from leoRst, for reference)
        def initCodeBlockString(self, p, language):
            ...
            if pygments and language in ('python', 'ruby', 'perl', 'c'):
                ...
                self.code_block_string = '**code**:\n\n.. class:: code\n..\n\n::\n\n'
        #@+node:ekr.20160331124028.45: *8* process_one_node
        def process_one_node(self, p, result, environment):
            ...
            if code and self.execcode:
                ...
                if err:
                    ...
                    result.append(err)
        #@+node:ekr.20160331124028.46: *8* exec_code
        ...
        #@+node:ekr.20160331124028.47: *8* format_output
        def format_output(self, s, prefix='::'):
            ...
            for line in lines:
                out += '    ' + line
            return out + '\n'
        #@+node:ekr.20160331124028.48: *8* process_directives
        def process_directives(self, s, d):
            ...
            return result, code
        #@+node:ekr.20160331124028.49: *8* underline2
        def underline2(self, p):
            ...
            return '%s\n%s\n\n' % (p.h, ch * n)
        #@+node:ekr.20160331124028.50: *8* write_rst
        def write_rst(self, root, s):
            ...
            f.close()
        #@+node:ekr.20160331124028.51: *4* md_render & helpers (md)
        def md_render(self):
            """Re-render the existing string, but probably with new configuration."""
            if self.rendering:
                # if already rendering, don't execute
                ...
                try:
                    ...
                    # No longer rendering, OK to receive another rendering call
                    self.rendering = False
        #@+node:ekr.20160331124028.52: *5* md_render_helper & helper
        def md_render_helper(self):
            ...
            if not self.auto:
                ...
                self.app.processEvents()
                self.pbar_action.setVisible(False)
        #@+node:ekr.20160331124028.53: *6* md_to_html & helper
        ...
        #@+node:ekr.20160331124028.54: *7* md_process_nodes & helpers
        def md_process_nodes(self, p, tree=True):
            ...
            return s
        #@+node:ekr.20160331124028.55: *8* md_code_directive
        def md_code_directive(self, lang):
            ...
            if pygments:
                ...
                return '\n'
        #@+node:ekr.20160331124028.56: *8* md_process_one_node
        def md_process_one_node(self, p, result, environment):
            ...
            if code and self.execcode:
                ...
                if err:
                    ...
                    result.append(err)
        #@+node:ekr.20160331124028.57: *8* md_exec_code
        def md_exec_code(self, code, environment):
            ...
            sys.stderr = saveerr # restore stderr
            return bufferout.getvalue(), buffererr.getvalue()
        #@+node:ekr.20160331124028.58: *8* md_format_output
        def md_format_output(self, s, prefix='```'):
            ...
            for line in lines:
                out += '    ' + line
            return out + '\n```\n'
        #@+node:ekr.20160331124028.59: *8* md_process_directives
        def md_process_directives(self, s, d):
            ...
            if trace: g.trace('result:\n', result) # ,'\ncode:',code)
            return result, code
        #@+node:ekr.20160331124028.60: *8* md_underline2
        def md_underline2(self, p):
            ...
            ch += ' ' + p.h
            return ch
        #@+node:ekr.20160331124028.61: *8* md_write_md
        def md_write_md(self, root, s):
            ...
            f.write(s.encode('utf8'))
            f.close()
        #@+node:ekr.20160331124028.62: *4* export
        def export(self):
        ...
        #@-others
    #@+node:ekr.20160331124028.63: *3* class ViewRenderedProvider
    class ViewRenderedProvider:
        #@+others
        #@+node:ekr.20160331124028.64: *4* __init__
        def __init__(self, c):
            self.c = c
            # Careful: we may be unit testing.
            if hasattr(c, 'free_layout'):
                ...
                if splitter:
                    splitter.register_provider(self)
        #@+node:ekr.20160331124028.65: *4* ns_provides
        def ns_provides(self):
            return [('Viewrendered2', '_leo_viewrendered')]
        #@+node:ekr.20160331124028.66: *4* ns_provide
        def ns_provide(self, id_):
            global controllers
            if id_ == '_leo_viewrendered':
                c = self.c
                ...
                return vr
        #@-others
    #@+node:ekr.20160331124028.67: *3* class ViewRenderedController (QWidget) (vr2)
    class ViewRenderedController(QtWidgets.QWidget):
        '''A class to control rendering in a rendering pane.'''
        #@+others
        #@+node:ekr.20160331124028.68: *4* vr2.ctor & helpers
        def __init__(self, c, parent=None):
            QtWidgets.QWidget.__init__(self, parent)
            self.setObjectName('viewrendered_pane')
            self.setLayout(QtWidgets.QVBoxLayout())
        ...
        #@+node:ekr.20160331124028.69: *5* vr2.create_dispatch_dict
        def create_dispatch_dict(self):
            pc = self
            pc.dispatch_dict = {
                'big': pc.update_rst,
        ...
        #@+node:ekr.20160331124028.70: *4* vr2.closeEvent
        def closeEvent(self, event):
            self.deactivate()
        #@+node:ekr.20160331124028.71: *4* vr2.contract & expand
        def contract(self):
            self.change_size(-100)
            ...
            if hasattr(self.c, 'free_layout'):
                ...
                for j in range(len(sizes)):
                    if j == i:
                        ...
                        sizes[j] = max(0, sizes[j] - int(delta / (n - 1)))
                splitter.setSizes(sizes)
        #@+node:ekr.20160331124028.72: *4* vr2.activate
        def activate(self):
            ...
            g.registerHandler('select2', pc.update)
            g.registerHandler('idle', pc.update)
        #@+node:ekr.20160331124028.73: *4* vr2.deactivate
        def deactivate(self):
            pc = self
            # Never disable the idle-time hook: other plugins may need it.
            g.unregisterHandler('select2', pc.update)
            g.unregisterHandler('idle', pc.update)
            pc.active = False
        #@+node:ekr.20160331124028.74: *4* vr2.lock/unlock
        def lock(self):
            g.note('rendering pane locked')
            self.locked = True
        ...
        def unlock(self):
            g.note('rendering pane unlocked')
            self.locked = False
        #@+node:ekr.20160331124028.75: *4* vr2.underline
        def underline(self, s):
            ch = '#'
            n = max(4, len(g.toEncodedString(s, reportErrors=False)))
            # return '%s\n%s\n%s\n\n' % (ch*n,s,ch*n)
            return '%s\n%s\n\n' % (s, ch * n)
        #@+node:ekr.20160331124028.76: *4* vr2.update & helpers
        # Must have this signature: called by leoPlugins.callTagHandler.
