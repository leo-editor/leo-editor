...
''' Provides an interface to the xdb command in a log pane.


The Debug Tab
=============

The plugin creates a Debug Tab in the log pane, consisting of a set of
icons and an input area.
...
#@-<< imports >>
#@+others
#@+node:ekr.20181004143535.4: ** init (xdb_pane.py)
def init():
...
#@+node:ekr.20181004143535.5: ** onCreate
def onCreate (tag,key):

    c = key.get('c')
    XdbController(c)
#@+node:ekr.20181004143535.6: ** popup_entry (not used)
if 0:
    def popup_entry(c,p,menu):
        c.cleo.addPopupMenu(c,p,menu)
#@+node:ekr.20181004143535.7: ** class XdbQtUI
if g.app.gui.guiName() == "qt":

    class XdbQtUI(QtWidgets.QWidget):
        #@+others
        #@+node:ekr.20181004143535.8: *3* ctor
        def __init__(self, owner, logTab=True):

            self.owner = owner
            QtWidgets.QWidget.__init__(self)
            uiPath = g.os_path_join(g.app.leoDir, 'plugins', 'XdbTab.ui')
            ...
            if logTab:
                self.owner.c.frame.log.createTab('Debug', widget = self)
            self.UI = form_class()
            ...
            for but in [
                ...
                'next_button', 'step_button', 'return_button',
                # "butPri1", "butPri6", "butPriChk", "butPri2",
            ...
            # self.UI.butDetails.clicked.connect(
                # lambda checked: self.UI.frmDetails.setVisible(
                    # not self.UI.frmDetails.isVisible()))
        ...
        #@+node:ekr.20181004143535.9: *3* make_func
        ...
        #@+node:ekr.20181004143535.10: *3* populateMenu
        @staticmethod
        ...
        #@+node:ekr.20181004143535.11: *3* setProgress
        ...
        #@+node:ekr.20181004143535.12: *3* setTime to be removed
...
#@+node:ekr.20181004143535.13: ** class XdbController
class XdbController(object):

    '''A per-commander class that manages tasks.'''

    #@+others
    #@+node:ekr.20181004143535.14: *3* priority table
    priorities = {
    ...
    #@+node:ekr.20181004143535.15: *3* __init__ & helper (todoController)
    def __init__ (self,c):
    ...
    #@+node:ekr.20181004143535.16: *4* reloadSettings (todoController)
    ...
    #@+node:ekr.20181004143535.17: *3* __del__
    def __del__(self):
        for i in self.handlers:
            g.unregisterHandler(i[0], i[1])
    #@+node:ekr.20181004143535.18: *3* _date
    def _date(self, d):
    ...
    #@+node:ekr.20181004143535.19: *3* addPopupMenu
    def addPopupMenu(self,c,p,menu):

        def rnd(x):
            return re.sub('.0$', '', '%.1f' % x)
        taskmenu = menu.addMenu("Debug")
        ...
        if isinstance(time_,float):
            ...
            a.enabled = False
        XdbQtUI.populateMenu(taskmenu, self)
    #@+node:ekr.20181004143535.20: *3* menuicon
    ...
    #@+node:ekr.20181004143535.21: *3* redrawer
    def redrawer(fn):
    ...
    #@+node:ekr.20181004143535.22: *3* projectChanger
    ...
    #@+node:ekr.20181004143535.25: *3* close
    ...
    #@+node:ekr.20181004143535.29: *4* hasUD
    ...
    #@+node:ekr.20181004143535.30: *4* getat
    ...
    #@+node:ekr.20181004143535.31: *4* testDefault
    ...
    #@+node:ekr.20181004143535.32: *4* setat
    ...
    #@+node:ekr.20181004143535.33: *4* dropEmpty
    def dropEmpty(self, node, dictOk = False):
        ...
        return False
    #@+node:ekr.20181004143535.34: *4* safe_del
    def safe_del(self, d, k):
        "delete a key from a dict. if present"
        if k in d: del d[k]
    #@+node:ekr.20181004143535.35: *3* drawing...
    #@+node:ekr.20181004143535.36: *4* redraw
    def redraw(self):
    ...
    #@+node:ekr.20181004143535.37: *4* clear_all
    @redrawer
    ...
    #@+node:ekr.20181004143535.38: *3* Progress/time/project...
    #@+node:ekr.20181004143535.39: *4* progress_clear
    @redrawer
    @projectChanger
    def progress_clear(self,v=None):

        self.setat(self.c.currentPosition().v, 'progress', '')
    #@+node:ekr.20181004143535.40: *4* set_progress
    @redrawer
    @projectChanger
    ...
    #@+node:ekr.20181004143535.41: *4* set_time_req
    @redrawer
    @projectChanger
    ...
    #@+node:ekr.20181004143535.42: *4* show_times
    @redrawer
    ...
    #@+node:ekr.20181004143535.43: *4* recalc_time
    def recalc_time(self, p=None, clear=False):
        ...
        return (time_totl, time_done)
    #@+node:ekr.20181004143535.44: *4* clear_time_req
    @redrawer
    @projectChanger
    def clear_time_req(self, p=None):
        ...
        v = p.v
        self.setat(v, 'time_req', '')
    #@+node:ekr.20181004143535.45: *4* update_project
    @redrawer
    ...
    #@+node:ekr.20181004143535.46: *4* local_recalc
    @redrawer
    def local_recalc(self, p=None):
        self.recalc_time(p)
    #@+node:ekr.20181004143535.47: *4* local_clear
    @redrawer
    def local_clear(self, p=None):
        self.recalc_time(p, clear=True)
    #@+node:ekr.20181004143535.48: *4* set_due_date
    def set_due_date(self,p=None, val=None, mode='adjust', field='duedate'):
        ...
        self.loadIcons(p)
    #@+node:ekr.20181004143535.49: *4* set_due_time
    ...
    #@+node:ekr.20181004143535.50: *4* set_date_offset
    def set_date_offset(self, field='nextworkdate'):
        ...
        p = self.c.currentPosition()
        self.loadIcons(p)
    #@+node:ekr.20181004143535.51: *3* ToDo icon related...
    #@+node:ekr.20181004143535.52: *4* childrenTodo
    @redrawer
    def childrenTodo(self, p=None):
        ...
        for p in p.children():
            ...
            self.loadIcons(p)
    #@+node:ekr.20181004143535.53: *4* dueClear
    @redrawer
    ...
    #@+node:ekr.20181004143535.54: *4* needs_doing
    ...
    #@+node:ekr.20181004143535.55: *4* find_todo
    @redrawer
    def find_todo(self, p=None, stage = 0):
        ...
        return False
    #@+node:ekr.20181004143535.56: *4* prikey
    def prikey(self, v):
    ...
    #@+node:ekr.20181004143535.57: *4* duekey
    ...
    #@+node:ekr.20181004143535.58: *4* dueSort
    @redrawer
    ...
    #@+node:ekr.20181004143535.59: *4* priority_clear
    @redrawer
    ...
    #@+node:ekr.20181004143535.60: *4* priSort
    @redrawer
    ...
    #@+node:ekr.20181004143535.61: *4* reclassify
    @redrawer
    def reclassify(self, p=None):
        ...
        self.showDist()
    #@+node:ekr.20181004143535.62: *4* setPri
    @redrawer
    def setPri(self,pri):
        ...
        self.loadIcons(p)
    #@+node:ekr.20181004143535.63: *4* showDist
    ...
    #@+node:ekr.20181004143535.64: *3* updateStyle
    ...
    #@+node:ekr.20181004143535.65: *3* updateUI
    def updateUI(self,tag=None,k=None):
        ...
        # )
    #@+node:ekr.20181004143535.66: *3* unl_to_pos
    def unl_to_pos(self, unl, for_p):
        ...
        return c2, nd
    #@-others
#@+node:ekr.20181004143535.67: ** command fix datetime
...
#@+node:ekr.20181004143535.68: ** command inc/dec priority
