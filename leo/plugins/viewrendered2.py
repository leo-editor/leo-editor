#@+leo-ver=5-thin
#@+node:pmills.20140316203745.5921: * @file viewrendered2.py
#@+<< docstring >>
...
class WebViewPlus(QtGui.QWidget):
    ...
    def to_html(self,p):
        ...
        # However, if self.s is not None then just use this as html
        # E.g. ScrolledTextMessage does this.
        # Otherwise html is generated by process_nodes (self.s ignored)
        if self.s:  html = self.s
        else:  html = self.process_nodes(self.pr,tree=self.tree)
        if not self.auto:  
