...
#@@wrap
# 
# I was trying to get Leo to work more effectively with Vim, my editor of choice. To do so, I made several changes to Leo which (I believe) make it work better.
# 
# After much exploring and trying various things, I made a change to the os.spawnv section of the openWith function in leoCommands.py. This added line seems to prevent the "weird error message on first open of Vim." (vim.py, line 32) when opening Vim with os.spawnv.
# 
...
# 
# Trying to execute locally If you use the ``--remote-silent`` argument, gVim will start the LEO server without the error message.
# 
# You can see which servers gVim has running by typing the following at the command prompt::
# 
#     vim --serverlist
# 
# The rest of my changes have to do with using the subprocess module instead of the os.system, and various os.spawn* calls. I find subprocess easier to understand, and it is fairly simple to use for the most common kinds of process calls, but is capable of all the variations you may need. It is designed to replace all the os.system, os.spawn, and popen calls. It is available in Python 2.4.
# 
