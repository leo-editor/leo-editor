...
#@-<< imports >>
#@+others
#@+node:ekr.20110605121601.18403: ** class LeoQtTree(leoFrame.leoTree)
class LeoQtTree(leoFrame.LeoTree):
    ...
    def initAfterLoad (self):
        ...
        # c.setChanged(False)
    #@+node:ekr.20110605121601.17871: *4* LeoQtTree.setConfigIvars
    ...
    #@+node:ekr.20110605121601.17940: *4* LeoQtTree.wrapQLineEdit
    def wrapQLineEdit (self,w):
        ...
        if isinstance(w,QtWidgets.QLineEdit):
            ...
            wrapper = w
        return wrapper
    #@+node:ekr.20140907201613.18985: *3* From BaseNativeTree
    # Some of these will be overridden
    #@+node:ekr.20110605121601.17885: *4* Event handlers... (nativeTree)
    #@+node:ekr.20110605121601.17886: *5* busy (nativeTree)
    def busy (self):
    ...
    #@+node:ekr.20110605121601.17887: *5* Click Box... (nativeTree)
    #@+node:ekr.20110605121601.17888: *6* onClickBoxClick
    def onClickBoxClick (self,event,p=None):
        ...
        c.outerUpdate()
    #@+node:ekr.20110605121601.17889: *6* onClickBoxRightClick
    def onClickBoxRightClick(self, event, p=None):
        ...
        c.outerUpdate()
    #@+node:ekr.20110605121601.17890: *6* onPlusBoxRightClick
    def onPlusBoxRightClick (self,event,p=None):
        ...
        c.outerUpdate()
    #@+node:ekr.20110605121601.17891: *5* Icon Box... (nativeTree)
    # For Qt, there seems to be no way to trigger these events.
    #@+node:ekr.20110605121601.17892: *6* onIconBoxClick (nativeTree)
    def onIconBoxClick (self,event,p=None):
        ...
        c.outerUpdate()
    #@+node:ekr.20110605121601.17893: *6* onIconBoxRightClick (nativeTree)
    def onIconBoxRightClick (self,event,p=None):
        ...
        c.outerUpdate()
    #@+node:ekr.20110605121601.17894: *6* onIconBoxDoubleClick (nativeTree)
    def onIconBoxDoubleClick (self,event,p=None):
        ...
        c.outerUpdate()
    #@+node:ekr.20110605121601.17895: *5* onItemCollapsed (nativeTree)
    ...
    def onItemDoubleClicked (self,item,col):
        ...
        if p:
            ...
            g.trace('*** no p')
        c.outerUpdate()
    #@+node:ekr.20110605121601.17898: *5* onItemExpanded (nativeTree)
    def onItemExpanded (self,item):
        ...
        c.outerUpdate()
    #@+node:ekr.20110605121601.17899: *5* onTreeSelect (nativeTree)
    def onTreeSelect(self):
        ...
        c.outerUpdate()
    #@+node:ekr.20110605121601.17900: *5* tree.OnPopup & allies (nativeTree)
    def OnPopup (self,p,event):
        ...
        return "break"
    #@+node:ekr.20110605121601.17901: *6* OnPopupFocusLost
    #@+at
    ...
    def OnPopupFocusLost(self,event=None):
        ...
        pass
    #@+node:ekr.20110605121601.17902: *6* createPopupMenu
    def createPopupMenu (self,event):

        '''This might be a placeholder for plugins.  Or not :-)'''
    #@+node:ekr.20110605121601.17903: *6* enablePopupMenuItems
    def enablePopupMenuItems (self,v,event):

        """Enable and disable items in the popup menu."""

    #@+node:ekr.20110605121601.17904: *6* showPopupMenu
    def showPopupMenu (self,event):

        """Show a popup menu."""
    #@+node:ekr.20110605121601.17905: *4* Selecting & editing... (nativeTree)
    #@+node:ekr.20110605121601.17906: *5* afterSelectHint (nativeTree)
    ...
    #@+node:ekr.20110605121601.17907: *5* beforeSelectHint (nativeTree)
    def beforeSelectHint (self,p,old_p):
        ...
        self.selecting = True
        self.prev_v = c.p.v
    #@+node:ekr.20110605121601.17908: *5* edit_widget (nativeTree)
    def edit_widget (self,p):
        ...
        if item:
            ...
            return None
    #@+node:ekr.20110605121601.17909: *5* editLabel (nativeTree)
    ...
    #@+node:ekr.20110605121601.17910: *5* editPosition (nativeTree)
    def editPosition(self):

        c = self.c ; p = c.currentPosition()
        ew = self.edit_widget(p)
        return ew and p or None
    #@+node:ekr.20110605121601.17911: *5* endEditLabel (nativeTree)
    ...
    def onHeadChanged (self,p,undoType='Typing',s=None,e=None):
        ...
        p.v.contentModified()
        c.outerUpdate()
    #@+node:ekr.20110605121601.17913: *5* setItemForCurrentPosition (nativeTree)
    def setItemForCurrentPosition (self,scroll=True):
        ...
        return item
    #@+node:ekr.20110605121601.17914: *5* setHeadline (nativeTree)
    ...
    #@+node:ekr.20110605121601.17915: *5* getSelectedPositions (nativeTree)
    def getSelectedPositions(self):
        items = self.getSelectedItems()
        pl = leoNodes.PosList(self.item2position(it) for it in items)
        return pl
    #@+node:ekr.20110605121601.17941: *4* Widget-independent helpers
    ...
    #@+node:ekr.20110605121601.17944: *5* Focus (nativeTree)
    def getFocus(self):
    ...
    #@+node:ekr.20110605121601.17945: *5* Icons (nativeTree)
    #@+node:ekr.20110605121601.17946: *6* drawItemIcon
    ...
    #@+node:ekr.20110605121601.17947: *6* getIconImage
    ...
    def getStatusIconImage (self,p):
        ...
        return r
    #@+node:ekr.20110605121601.17949: *6* getVnodeIcon
    def getVnodeIcon(self,p):

        '''Return the proper icon for position p.'''

        return self.getIcon(p)
    #@+node:ekr.20110605121601.17950: *6* setItemIcon (nativeTree)
        ...
        if icon and valid:
            ...
            if icon:
                g.trace('** item %s, valid: %s, icon: %s' % (
                    ...
                    g.callers(4))
    #@+node:ekr.20110605121601.17951: *6* updateIcon (nativeTree)
    def updateIcon (self,p,force=False):
        ...
        for item in items:
            self.setItemIcon(item,icon)
    #@+node:ekr.20110605121601.17952: *6* updateVisibleIcons (nativeTree)
    ...
    #@+node:ekr.20110605121601.18408: *4* clear
    def clear (self):
    ...
    #@+node:ekr.20140907201613.18986: *4* repaint
    def repaint (self):
        '''Repaint the widget.'''
        w = self.treeWidget
        w.repaint()
        w.resizeColumnToContents(0) # 2009/12/22
    #@+node:ekr.20110605121601.18409: *3* Icons (LeoQtTree)
    #@+node:ekr.20110605121601.18410: *4* drawIcon
    def drawIcon (self,p):
    ...
    #@+node:ekr.20110605121601.18411: *4* getIcon & helper (qtTree)
    def getIcon(self,p):
    ...
    #@+node:ekr.20110605121601.18412: *5* getCompositeIconImage
    def getCompositeIconImage(self,p,val):
        ...
        return icon
    #@+node:ekr.20110605121601.18413: *4* setItemIconHelper (qtTree)
    def setItemIconHelper (self,item,icon):
        ...
        if item:
            item.setIcon(0,icon)
    #@+node:ekr.20110605121601.18414: *3* Items (LeoQtTree)
    #@+node:ekr.20110605121601.18415: *4* childIndexOfItem
    ...
    #@+node:ekr.20110605121601.18416: *4* childItems
    def childItems (self,parent_item):
        ...
        return items
    #@+node:ekr.20110605121601.18417: *4* closeEditorHelper (LeoQtTree)
        ...
        if e:
            ...
            try:
                ...
                if 1: # Testing.
                    ...
                    pass
    #@+node:ekr.20110605121601.18418: *4* connectEditorWidget & helper
    ...
    #@+node:ekr.20110605121601.18419: *4* contractItem & expandItem
    ...
    #@+node:ekr.20110605121601.18420: *4* createTreeEditorForItem (LeoQtTree)
    def createTreeEditorForItem(self,item):
        ...
        return e,wrapper
    #@+node:ekr.20110605121601.18421: *4* createTreeItem
    ...
    #@+node:ekr.20110605121601.18423: *4* getCurrentItem
    def getCurrentItem (self):

        w = self.treeWidget
        return w.currentItem()
    #@+node:ekr.20110605121601.18424: *4* getItemText
    def getItemText (self,item):
        ...
        if item:
            ...
            return '<no item>'
    #@+node:ekr.20110605121601.18425: *4* getParentItem
    def getParentItem(self,item):

        return item and item.parent()
    #@+node:ekr.20110605121601.18426: *4* getSelectedItems
    def getSelectedItems(self):
        w = self.treeWidget    
        return w.selectedItems()
    #@+node:ekr.20110605121601.18427: *4* getTreeEditorForItem (LeoQtTree)
    def getTreeEditorForItem(self,item):
        ...
        return e
    #@+node:ekr.20110605121601.18428: *4* getWrapper (LeoQtTree)
    def getWrapper (self,e,item):
        ...
        if e:
            ...
            g.trace('no e')
            return None
    #@+node:ekr.20110605121601.18429: *4* nthChildItem
    def nthChildItem (self,n,parent_item):
        ...
        return item
    #@+node:ekr.20110605121601.18430: *4* scrollToItem (LeoQtTree)
    def scrollToItem (self,item):
        ...
        self.setHScroll(0)
    #@+node:ekr.20110605121601.18431: *4* setCurrentItemHelper (LeoQtTree)
    def setCurrentItemHelper(self,item):

        w = self.treeWidget
        w.setCurrentItem(item)
    #@+node:ekr.20110605121601.18432: *4* setItemText
    def setItemText (self,item,s):

        if item:
            item.setText(0,s)
    #@+node:ekr.20110605121601.18433: *3* Scroll bars (LeoQtTree)
    #@+node:ekr.20110605121601.18434: *4* getSCroll
    def getScroll (self):
        ...
        return hPos,vPos
    #@+node:ekr.20110605121601.18435: *4* setH/VScroll
    ...
    #@+node:btheado.20111110215920.7164: *4* scrollDelegate (LeoQtTree)
    def scrollDelegate (self,kind):
        ...
        c.treeWantsFocus()
    #@+node:ekr.20110605121601.18437: *3* onContextMenu (LeoQtTree)
