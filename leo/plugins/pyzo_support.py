...
def open_file_browser(use_main_imports, use_pyzo_config, use_pyzo_logging):
 ...
 # use_config
        ...
        g.es_exception()
        return None
#@+node:ekr.20190413082316.1: ** function: open_shell_window
def open_pyzo_shell(use_pyzo_config, use_pyzo_logging):
    '''A test bed for importing pyzo's file browser.'''
    # Prefer explicit imports to implicit.
    import sys
    #@+<< set g.pyzo switches >>
    #@+node:ekr.20190414034315.3: *3* << set g.pyzo switches >>
    if '--pyzo' not in sys.argv:
        sys.argv.append('--pyzo')
    g.pyzo = True
    g.pyzo_pdb = False
    ...
    #@-<< set g.pyzo switches >>
    try:
        #@+<< pyzo/__init__.py sources >>
        #@+node:ekr.20190414034315.4: *3* << pyzo/__init__.py sources >>
        #@+<< pyzo/__init__.py imports >>
        #@+node:ekr.20190414034315.5: *4* << pyzo/__init__.py imports >>
        import os
        # import sys #EKR: imported above.
        import locale
        import traceback
        ...
        if 0: # Pyflakes.
            print(QtCore, QtGui, QtWidgets, translate, setLanguage)
        #@-<< pyzo/__init__.py imports >>
        #@+<< pyzo.__init__.py early bindings >>
        #@+node:ekr.20190414034315.6: *4* << pyzo.__init__.py early bindings >>
        # Set environ to let kernel know some stats about us
        os.environ['PYZO_PREFIX'] = sys.prefix
        _is_pyqt4 = hasattr(QtCore, 'PYQT_VERSION_STR')
        os.environ['PYZO_QTLIB'] = 'PyQt4' if _is_pyqt4 else 'PySide'
        #@-<< pyzo.__init__.py early bindings >>
        #@+others
        #@+node:ekr.20190414034315.8: *4* getResourceDirs
        def getResourceDirs():
            """ getResourceDirs()
            Get the directories to the resources: (pyzoDir, appDataDir).
            Also makes sure that the appDataDir has a "tools" directory and
            ...
            return pyzoDir, appDataDir
        #@+node:ekr.20190414034315.9: *4* loadConfig (pyzo.__init__.py)
        def loadConfig(defaultsOnly=False):
            """ loadConfig(defaultsOnly=False)
            Load default and site-wide configuration file(s) and that of the user (if it exists).
            Any missing fields in the user config are set to the defaults.
        ...
        #@-others
        #@+<< pyzo.__init__.py late bindings >>
        #@+node:ekr.20190414034315.7: *4* << pyzo.__init__.py late bindings >>
        ## Init
        ...
        if 0: # Keep pyflakes happy.
            print(editors, shells, main, icon, parser, status)
        ...
        # Create ssdf in module namespace, and fill it
        if use_pyzo_config:
            _saveConfigFile = False ### Was True
            config = ssdf.new()
            loadConfig()
        else:
            config = ConfigShim() # g.TracingNullObject(tag='config shim')
        ...
        defaultQtStyleName = ''
        if 0:
            print(_saveConfigFile, defaultQtStyleName)
        #@-<< pyzo.__init__.py late bindings >>
        #@-<< pyzo/__init__.py sources >>
        if use_pyzo_logging:
            # Importing this module actually starts logging.
            ...
            assert pyzoLogging
        #@+<< import the shell >>
        #@+node:ekr.20190414034315.10: *3* << import the shell >>
        #@+<< unused imports >>
        #@+node:ekr.20190414034315.11: *4* << unused imports >>
        # import pyzo.tools as tools
        # assert tools
        ...
        #@-<< import the shell >>
        shell = ShellStackWidget(parent=None)
        shell.show()
        return shell
    except Exception:
        g.es_exception()
...
w = open_pyzo_shell(
    use_pyzo_config=True, # Works either way.
    use_pyzo_logging=False, # Works either way.
)
g.trace(w)
g.app.permanentScriptDict ['pyzo-shell'] = w
    # Required.
#@-others
#@-leo
