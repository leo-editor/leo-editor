...
#@+others
#@+node:ekr.20211209153614.1: ** do_import
def do_import(c, s, parent):
    return Py_Importer(c.importCommands).run(s, parent)
#@+node:ekr.20211209153303.2: ** class Py_Importer(Importer)
class Py_Importer(Importer):
    """A class to store and update scanning state."""
    
    #@+<< Py_Importer debug vars >>
    #@+node:ekr.20211209153303.3: *3* << Py_Importer debug vars >>
    ...
    #@+others
    #@+node:ekr.20211209153303.4: *3* pi_i.check & helper
    def check(self, unused_s, parent):
        ...
        return ok
    #@+node:ekr.20211209153303.5: *4* pi_i.strip_blank_and_comment_lines
    ...
    #@+node:ekr.20211209153303.6: *3* py_i.clean_headline
        ...
        if g.unitTesting or self.put_decorators:
            for s in self.get_lines(p):
                if not s.isspace():
                    ...
                    return ''
        return ''
    #@+node:ekr.20211209153303.7: *3* py_i.find_class & helper
    def find_class(self, parent):
        ...
        return None, -1, -1
    #@+node:ekr.20211209153303.8: *4* py_i.skip_block (*** changed)
    def skip_block(self, i, index, lines, prev_state, stack):
        ...
        while i < len(lines):
            ...
            i += 1
        return None, -1, -1
    #@+node:ekr.20211209153303.9: *3* py_i.gen_lines & helpers
    ...
    def gen_lines(self, lines, parent):
        ...
        for line in lines: ### g.splitLines(s):
    ...
    #@+node:ekr.20211209153303.10: *4* py_i.do_class
    def do_class(self, line, parent):
        ...
        return p
    #@+node:ekr.20211209153303.11: *4* py_i.do_def
    ...
    #@+node:ekr.20211209153303.12: *4* py_i.do_normal_line
    def do_normal_line(self, line, p):
        ...
        return p
    #@+node:ekr.20211209153303.13: *4* py_i.end_previous_blocks
    ...
    #@+node:ekr.20211209153303.14: *4* py_i.gen_python_ref
    ...
    #@+node:ekr.20211209153303.15: *4* py_i.start_python_block
    def start_python_block(self, kind, line, parent):
        ...
        return p
    #@+node:ekr.20211209153303.16: *4* py_i: explicit post-pass (to do)
    #@+node:ekr.20211209153303.17: *5* py_i.adjust_all_decorator_lines & helper
    ...
    def adjust_decorator_lines(self, p):
        ...
        ### To do
    #@+node:ekr.20211209153303.18: *5* py_i.promote_first_child (to do)
    def promote_first_child(self, parent):
        """Move a smallish first child to the start of parent."""
    #@+node:ekr.20211209153303.19: *4* py_i: overrides
    #@+node:ekr.20211209153303.20: *5* py_i: do-nothing overrides
    #@+node:ekr.20211209153303.21: *6* py_i.create_child_node (do-nothing)
    def create_child_node(self, parent, line, headline):
        """Create a child node of parent."""
        assert False, g.callers()
        
    #@+node:ekr.20211209153303.22: *6* py_i.cut_stack (do-nothing)
    def cut_stack(self, new_state, stack):
        """Cut back the stack until stack[-1] matches new_state."""
        assert False, g.callers()
    #@+node:ekr.20211209153303.23: *6* py_i.trace_status (do-nothing)
    def trace_status(self, line, new_state, prev_state, stack, top):
        """Do-nothing override of Import.trace_status."""
        assert False, g.callers()
    #@+node:ekr.20211209153303.24: *5* py_i.add_line (tracing version)
    ...
    #@+node:ekr.20211209153303.25: *5* py_i.common_lws
    ...
    #@+node:ekr.20211209153303.26: *5* py_i: i.post_pass overrides
    #@+node:ekr.20211209153303.27: *6* py_i.clean_all_headlines
    def clean_all_headlines(self, parent):
        ...
        for p in parent.subtree():
            ...
            if h and h != p.h:
                p.h = h
    #@+node:ekr.20211209153303.28: *6* py_i.find_tail (not used yet)
    ...
    #@+node:ekr.20211209153303.29: *6* py_i.promote_last_lines
    def promote_last_lines(self, parent):
        """A do-nothing override."""
    #@+node:ekr.20211209153303.30: *6* py_i.promote_trailing_underindented_lines (do-nothing override)
    def promote_trailing_underindented_lines(self, parent):
        """A do-nothing override."""
        
    #@+node:ekr.20211209153303.31: *3* py_i.get_new_dict
    #@@nobeautify
    ...
    def get_new_dict(self, context):
        ...
        return d
    #@-others
#@+node:ekr.20211209153303.32: ** class Python_ScanState
class Python_ScanState:
    ...
    #@+others
    #@+node:ekr.20211209153303.33: *3* py_state.__repr__ & short_description
    def __repr__(self):
        """Py_State.__repr__"""
    ...
    #@+node:ekr.20211209153303.34: *3* py_state.level
    def level(self):
        """Python_ScanState.level."""
        return self.indent
    #@+node:ekr.20211209153303.35: *3* py_state.in_context
    def in_context(self):
    ...
    #@+node:ekr.20211209153303.36: *3* py_state.update
