...
class Py_Importer(Importer):
    ...
    #@+others
    #@+node:ekr.20211128161017.3: *3* py_i.clean_headline
        ...
        if g.unitTesting or self.put_decorators:
            for s in self.get_lines(p):
                if not s.isspace():
                    ...
                    return ''
        return ''
    #@+node:ekr.20211128161017.4: *3* py_i.find_class & helper
    def find_class(self, parent):
        ...
        for i, line in enumerate(lines):
            ...
            index += len(line)
        return None, -1, -1
    #@+node:ekr.20211128161017.5: *4* py_i.skip_block
    ...
    #@+node:ekr.20211128161017.7: *4* py_i.common_lws
    ...
    #@+node:ekr.20211128161017.8: *4* py_i.cut_stack
    ...
    #@+node:ekr.20211128161017.9: *4* py_i.end_block
    def end_block(self, i, lines, new_state, prev_state, stack):
        ...
        return top.p
    #@+node:ekr.20211128161017.10: *4* py_i.ends_block
    ...
    #@+node:ekr.20211128161017.11: *4* py_i.gen_ref
    def gen_ref(self, line, parent, target):
        ...
        return h
    #@+node:ekr.20211128161017.12: *4* py_i.start_new_block
    def start_new_block(self, i, lines, new_state, prev_state, stack):
        ...
        stack.append(target)
    #@+node:ekr.20211128161017.13: *4* py_i.starts_block
    ...
    def starts_block(self, i, lines, new_state, prev_state, stack):
        ...
        if top.at_others_flag and new_state.indent > prev_indent:
            return False
        return True
    #@+node:ekr.20211128161017.14: *4* py_i.starts_decorator
    ...
    def starts_decorator(self, i, lines, prev_state):
        ...
        return False
    #@+node:ekr.20211128161017.15: *3* py_i.get_new_dict
    #@@nobeautify
    ...
    def get_new_dict(self, context):
        ...
        return d
    #@+node:ekr.20211128161017.16: *3* py_i: post_pass
    #@+node:ekr.20211128161017.17: *4* py_i.clean_all_headlines
    def clean_all_headlines(self, parent):
        ...
        for p in parent.subtree():
            ...
            if h and h != p.h:
                p.h = h
    #@+node:ekr.20211128161017.18: *4* py_i.promote_last_lines
    def promote_last_lines(self, parent):
        ...
        for i, line in enumerate(lines):
            ...
            if m and not prev_state.context and new_state.indent == 0:
                ...
                prev_state = new_state
    #@+node:ekr.20211128161017.19: *4* py_i.promote_trailing_underindented_lines
    ...
    #@-others
#@+node:ekr.20211128161017.20: ** class Python_ScanState
class Python_ScanState:
    ...
    #@+others
    #@+node:ekr.20211128161017.21: *3* py_state.__repr__
    def __repr__(self):
        """Py_State.__repr__"""
    ...
    __str__ = __repr__
    #@+node:ekr.20211128161017.22: *3* py_state.level
    def level(self):
        """Python_ScanState.level."""
        return self.indent
    #@+node:ekr.20211128161017.23: *3* py_state.in_context
    def in_context(self):
    ...
    #@+node:ekr.20211128161017.24: *3* py_state.update
    ...
    #@-others
#@+node:ekr.20211128161017.25: ** class PythonTarget
class PythonTarget:
    """
