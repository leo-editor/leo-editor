...
class Cython_Importer(Importer):
    ...
    #@+others
    #@+node:ekr.20200619141201.3: *3* py_i.clean_headline
        ...
        if g.unitTesting or self.put_decorators:
            for s in self.get_lines(p):
                if not s.isspace():
                    ...
                    return ''
        return ''
    #@+node:ekr.20200619141201.4: *3* py_i.find_class & helper
    def find_class(self, parent):
        ...
        for i, line in enumerate(lines):
            ...
            index += len(line)
        return None, -1, -1
    #@+node:ekr.20200619141201.5: *4* py_i.skip_block
    def skip_block(self, i, index, lines, prev_state, stack):
        ...
        while i < len(lines):
            ...
            assert progress < i
        return None, -1, -1
    #@+node:ekr.20200619141201.6: *3* py_i.gen_lines & overrides
    def gen_lines(self, s, parent):
        ...
        target = CythonTarget(parent, prev_state)
        stack = [target, target]
        ...
        for i, line in enumerate(lines):
            ...
            if self.skip > 0:
                ...
                if self.is_ws_line(line):
                    ...
                    stack.append(CythonTarget(p, new_state))
    ...
    #@+node:ekr.20200619141201.7: *4* py_i.common_lws
    ...
    #@+node:ekr.20200619141201.8: *4* py_i.cut_stack
    def cut_stack(self, new_state, stack, append=False):
        ...
        while stack:
            ...
            if new_state.level() < top_state.level():
                ...
                # This happens often in valid cython programs.
                break
        # Restore the guard entry if necessary.
        if len(stack) == 1:
            stack.append(stack[-1])
        assert len(stack) > 1 # Fail on exit.
    #@+node:ekr.20200619141201.9: *4* py_i.end_block
    def end_block(self, i, lines, new_state, prev_state, stack):
        ...
        return top.p
    #@+node:ekr.20200619141201.10: *4* py_i.ends_block
    def ends_block(self, line, new_state, prev_state, stack):
        '''True if line ends the block.'''
        # Comparing new_state against prev_state does not work for cython/python.
    ...
    #@+node:ekr.20200619141201.11: *4* py_i.gen_ref
    def gen_ref(self, line, parent, target):
        ...
        return h
    #@+node:ekr.20200619141201.12: *4* py_i.start_new_block
    def start_new_block(self, i, lines, new_state, prev_state, stack):
        ...
        self.decorator_lines = []
        target = CythonTarget(child, new_state)
        target.kind = 'class' if h.startswith('class') else 'def'
        stack.append(target)
    #@+node:ekr.20200619141201.13: *4* py_i.starts_block
    ...
    def starts_block(self, i, lines, new_state, prev_state, stack):
        ...
        if top.at_others_flag and new_state.indent > prev_indent:
            return False
        return True
    #@+node:ekr.20200619141201.14: *4* py_i.starts_decorator
    ...
    def starts_decorator(self, i, lines, prev_state):
        ...
        return False
    #@+node:ekr.20200619141201.15: *3* py_i.get_new_dict
    #@@nobeautify
    ...
    def get_new_dict(self, context):
        ...
        return d
    #@+node:ekr.20200619141201.16: *3* py_i: post_pass
    #@+node:ekr.20200619141201.17: *4* py_i.clean_all_headlines
    def clean_all_headlines(self, parent):
        ...
        for p in parent.subtree():
            ...
            if h and h != p.h:
                p.h = h
    #@+node:ekr.20200619141201.18: *4* py_i.promote_last_lines
    def promote_last_lines(self, parent):
        '''cython_i.promote_last_lines.'''
        ...
        for i, line in enumerate(lines):
            ...
            if m and not prev_state.context and new_state.indent == 0:
                ...
                prev_state = new_state
    #@+node:ekr.20200619141201.19: *4* py_i.promote_trailing_underindented_lines
    ...
    #@-others
#@+node:ekr.20200619141201.20: ** class Cython_ScanState
class Cython_ScanState:
    '''A class representing the state of the cython line-oriented scan.'''
    ...
    #@+others
    #@+node:ekr.20200619141201.21: *3* py_state.__repr__
    def __repr__(self):
        '''Py_State.__repr__'''
    ...
    __str__ = __repr__
    #@+node:ekr.20200619141201.22: *3* py_state.level
    def level(self):
        '''Cython_ScanState.level.'''
        return self.indent
    #@+node:ekr.20200619141201.23: *3* py_state.in_context
    def in_context(self):
    ...
    #@+node:ekr.20200619141201.24: *3* py_state.update
    ...
    #@-others
#@+node:ekr.20200619141201.25: ** class CythonTarget
class CythonTarget:
    '''
