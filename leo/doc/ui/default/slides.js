...
function keys(key) {
	...
	if (s5mode) {
		...
		if (key.which < 48 || key.which > 57) {
			...
			if (window.event && isParentOrSelf(window.event.srcElement, 'controls')) return; /* ekr: was return; */
			if (key.target && isParentOrSelf(key.target, 'controls')) return; /* ekr: was return; */
