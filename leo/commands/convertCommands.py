...
#@+<< imports >>
#@+node:ekr.20160316095341.1: ** << imports >> (convertCommands.py)
...
class ConvertCommandsClass(BaseEditCommandsClass):
    '''Leo's file-conversion commands'''
    #@+others
    #@+node:ekr.20160316095557.1: ** ctor (ConvertCommandsClass)
    def __init__(self, c):
        '''Ctor for EditCommandsClass class.'''
        # pylint: disable=super-init-not-called
        self.c = c

    #@+node:ekr.20150514063305.121: ** c-to-python & typescript-to-py
    #@+<< class To_Python >>
    ...
    #@-<< class TS_To_Python (To_Python) >>
    #@+others
    #@+node:ekr.20160316091843.1: *3* c-to-python
    @cmd('c-to-python')
    ...
    #@+node:ekr.20160316091843.2: *3* typescript-to-py
    ...
    def tsToPy(self, event):
        ...
        c.bodyWantsFocus()
    #@-others
    #@+node:ekr.20160316091923.1: ** python-to-coffeescript
    ...
    def python2coffeescript(self, event):
        ...
        '''
        #@+others
        #@+node:ekr.20160316092837.1: *3* class Python_To_Coffeescript_Adapter
        class Python_To_Coffeescript_Adapter:
            ...
            def __init__(self,c):
                ...
                self.c = c
                self.py2cs = py2cs = g.importExtension('py2cs',
                    pluginName=None, verbose=False, required=False)

            #@+others
            #@+node:ekr.20160316093019.1: *4* 2cs.main
            def main(self):
                '''Main line for Python_To_CoffeeScript class.'''
                if self.py2cs:
                    self.run()
                    g.es_print('python-to-coffeescript: not ready yet')
                else:
                    g.es_print('can not load py2cs.py')
                   
                
            #@+node:ekr.20160316094011.3: *4* MSF_ctor & helpers
            def MSF__init__(self, c):
            ...
            #@+node:ekr.20160316094011.4: *5* 2cs.scan
            def scan(self, kind):
                ...
                return aList
            #@+node:ekr.20160316094011.5: *5* 2cs.scan_d
            def scan_d(self, kind):
                ...
                return d
            #@+node:ekr.20160316094011.7: *4* 2cs.finalize
            def finalize(self, fn):
                '''Finalize and regularize a filename.'''
                return g.os_path_normpath(g.os_path_abspath(g.os_path_expanduser(fn)))
            #@+node:ekr.20160316094011.8: *4* 2cs.to_coffeescript
            def to_coffeescript(self, p):
                ...
                # x.run(node)
            #@+node:ekr.20160316094011.9: *4* 2cs.run
            def run(self):
