...
class LocalConfigManager:
    ...
    def settingIsActiveInPath(self, gs, target_path):
        ...
        assert isinstance(gs, g.GeneralSetting), repr(gs)
        if not gs.path:
            # g.trace('NO PATH', target_path, repr(gs))
            return False
        if gs.path != target_path:
            ### g.trace('OOPS: path mismatch\n', gs.path, '\n', target_path)
            return False
        if target_path == c.fileName():
            # A local file.
            return True
        if target_path == lm.theme_path:
            return True
        if target_path == lm.my_settings_path:
            return True
        if target_path == lm.leo_settings_path:
            return True
        return False
        ...
        ### From getSource
            ...
            # if not path:
                # g.trace('NO PATH', gs.kind, g.truncate(repr(gs.val), 50))
                # return False ### ???"local_file"
            # path = path.lower()
            # for tag in ('myLeoSettings.leo', 'leoSettings.leo'):
                # if path.endswith(tag.lower()):
                    # return tag.rstrip('.leo')
            # theme_path = g.app.loadManager.theme_path
            # if theme_path and g.shortFileName(theme_path.lower()) in path:
                # g.trace('FOUND:', "theme_file", gs.kind, gs.ivar, g.truncate(repr(gs.val), 50))
                # return "theme_file"
            # # g.trace('NOT FOUND', repr(theme_path), repr(path))
            # if path == 'register-command' or path.find('mode') > -1:
                # return 'ignore'
        ...
        ### From create_unified_settings: < <handle a real setting> >
            # d = self.filter_settings(kind)
            # key = g.app.config.munge(m.group(2).strip())
            # val = d.get(key)
            # if isinstance(val, g.GeneralSetting):
