#@+leo-ver=5-thin
#@+node:ekr.20240312053644.1: * @file leoMypycTokens.py
...
#@+<< leoMypycTokens.py: docstring >>
#@+node:ekr.20240312053644.2: ** << leoMypycTokens.py: docstring >>
"""
...
#@+node:ekr.20240312053644.3: ** << leoMypycTokens.py: imports & annotations >>
...
#@+others
#@+node:ekr.20240312053644.4: ** top-level functions (leoMypycTokens.py)
#@+node:ekr.20240312053644.5: *3* function: check_g
def check_g() -> bool:  # pragma: no cover
    ...
    return bool(g)
#@+node:ekr.20240312053644.6: *3* function: dump_contents
def dump_contents(contents: str, tag: str = 'Contents') -> None:  # pragma: no cover
    ...
    print('')
#@+node:ekr.20240312053644.7: *3* function: dump_lines
...
def dump_tokens(tokens: list[InputToken], tag: str = 'Tokens') -> None:  # pragma: no cover
    ...
    for z in tokens:
        print(z.dump())
    print('')
#@+node:ekr.20240312053644.10: *3* function: get_modified_files
def get_modified_files(repo_path: str) -> list[str]:  # pragma: no cover
    ...
    try:
        ...
        os.chdir(old_cwd)
#@+node:ekr.20240312053644.11: *3* function: input_tokens_to_string
...
#@+node:ekr.20240312053644.12: *3* function: main (leoTokens.py)
...
#@+node:ekr.20240312053644.13: *3* function: orange_command (leoTokens.py)
def orange_command(
    arg_files: list[str],
    requested_files: list[str],
...
#@+node:ekr.20240312053644.14: *3* function: output_tokens_to_string
def output_tokens_to_string(tokens: list[OutputToken]) -> str:
    ...
    if tokens is None:  # pragma: no cover
        ...
        return ''
    return ''.join([z.to_string() for z in tokens])
#@+node:ekr.20240312053644.15: *3* function: scan_args (leoTokens.py)
...
#@+node:ekr.20240312053644.16: ** Classes (leoMypycTokens.py)
#@+node:ekr.20240312053644.17: *3* class InternalBeautifierError(Exception)
class InternalBeautifierError(Exception):
    ...
    """
#@+node:ekr.20240312053644.18: *3* class InputToken
class InputToken:  # leoTokens.py.
    ...
    #@+others
    #@+node:ekr.20240312053644.19: *4* itoken.brief_dump
    ...
    #@+node:ekr.20240312053644.20: *4* itoken.dump
    ...
    #@+node:ekr.20240312053644.21: *4* itoken.dump_header
    def dump_header(self) -> None:  # pragma: no cover
        ...
        print(
            ...
            f"==== ===== ===== {'':10} ===== {'':10} ==== =====\n")
    #@+node:ekr.20240312053644.22: *4* itoken.error_dump
    ...
    #@+node:ekr.20240312053644.23: *4* itoken.show_val
    def show_val(self, truncate_n: int = 8) -> str:  # pragma: no cover
        ...
        return val
    #@-others
#@+node:ekr.20240312053644.24: *3* class Tokenizer
class Tokenizer:
    """
    ...
    #@+others
    #@+node:ekr.20240312053644.25: *4* Tokenizer.add_token
    ...
    #@+node:ekr.20240312053644.26: *4* Tokenizer.check_results
    ...
    #@+node:ekr.20240312053644.27: *4* Tokenizer.check_round_trip
    def check_round_trip(self, contents: str, tokens: list[InputToken]) -> bool:
        ...
        if not ok:  # pragma: no cover
            ...
            g.printObj(result)
        return ok
    #@+node:ekr.20240312053644.28: *4* Tokenizer.create_input_tokens
    ...
    #@+node:ekr.20240312053644.29: *4* Tokenizer.do_token (the gem)
    ...
    #@+node:ekr.20240312053644.30: *4* Tokenizer.make_input_tokens (entry)
    def make_input_tokens(self, contents: str) -> list[InputToken]:
        ...
        return tokens
    #@+node:ekr.20240312053644.31: *4* Tokenizer.tokens_to_string
    def tokens_to_string(self, tokens: list[InputToken]) -> str:
        ...
        return ''.join([z.to_string() for z in tokens])
    #@-others
#@+node:ekr.20240312053644.32: *3* class OutputToken
class OutputToken:
    """
    ...
    #@+others
    #@+node:ekr.20240312053644.33: *4* otoken.show_val
    def show_val(self, truncate_n: int) -> str:  # pragma: no cover
        ...
        return val
    #@-others
#@+node:ekr.20240312053644.34: *3* class ParseState
class ParseState:
    """
...
class TokenBasedOrange:  # Orange is the new Black.
    ...
    #@+node:ekr.20240312053644.37: *4* << TokenBasedOrange: docstring >>
    #@@language rest
    #@@wrap
    ...
    **Design**
    ...
    #@+node:ekr.20240312053644.38: *4* << TokenBasedOrange: __slots__ >>
    __slots__ = [
    ...
    def dump_token_range(self, i1: int, i2: int, tag: str = None) -> None:  # pragma: no cover
        ...
        for token in self.tokens[i1 : i2 + 1]:
            print(token.dump())
    #@+node:ekr.20240312053644.44: *5* tbo.internal_error_message
    ...
    #@+node:ekr.20240312053644.45: *5* tbo.user_error_message
    ...
    #@+node:ekr.20240312053644.46: *5* tbo.oops
    ...
    #@+node:ekr.20240312053644.47: *4* tbo: Entries & helpers
    #@+node:ekr.20240312053644.48: *5* tbo.beautify (main token loop)
    ...
    def beautify(self,
        ...
        #@+node:ekr.20240312053644.49: *6* << tbo.beautify: init ivars >>
        # Debugging vars...
        self.contents = contents
    ...
    #@+node:ekr.20240312053644.50: *5* tbo.beautify_file (entry) (stats & diffs)
    def beautify_file(self, filename: str, was_dirty: bool) -> bool:  # pragma: no cover
        ...
        return True
    #@+node:ekr.20240312053644.51: *5* tbo.init_tokens_from_file
    ...
    #@+node:ekr.20240312053644.53: *5* tbo.show_diffs
    ...
    #@+node:ekr.20240312053644.57: *5* tbo.do_dedent
    ...
    #@+node:ekr.20240312053644.58: *5* tbo.do_encoding
    def do_encoding(self) -> None:
        """Handle the encoding token."""
    #@+node:ekr.20240312053644.59: *5* tbo.do_endmarker
    ...
    #@+node:ekr.20240312053644.60: *5* tbo.do_indent
    ...
    def do_indent(self) -> None:
        ...
        self.lws = new_indent
        self.gen_line_indent()
    #@+node:ekr.20240312053644.61: *5* tbo.do_name & generators
    ...
    #@+node:ekr.20240312053644.62: *6* tbo.gen_word
    def gen_word(self, s: str) -> None:
        ...
        self.gen_blank()
    #@+node:ekr.20240312053644.63: *6* tbo.gen_word_op
    def gen_word_op(self, s: str) -> None:
        ...
        self.gen_blank()
    #@+node:ekr.20240312053644.64: *5* tbo.do_newline, do_nl & generators
    ...
    def gen_line_end(self) -> OutputToken:
        ...
        return t
    #@+node:ekr.20240312053644.66: *6* tbo.gen_line_indent
    ...
    #@+node:ekr.20240312053644.67: *5* tbo.do_number
    def do_number(self) -> None:
        """Handle a number token."""
        self.gen_blank()
        self.gen_token('number', self.token.value)
    #@+node:ekr.20240312053644.68: *5* tbo.do_op & generators
    def do_op(self) -> None:
        """Handle an op token."""
        val = self.token.value
        if val == '.':
            ...
            self.gen_token('op', val)
            self.gen_blank()
    #@+node:ekr.20240312053644.69: *6* tbo.gen_colon & helper
    def gen_colon(self) -> None:
        ...
        if context == 'complex-slice':
            ...
            self.gen_token('op', val)
            self.gen_blank()
    #@+node:ekr.20240312053644.70: *6* tbo.gen_dot_op
    ...
    def gen_equal_op(self) -> None:
        ...
        if context == 'initializer':
            ...
            self.gen_token('op', val)
            self.gen_blank()
    #@+node:ekr.20240312053644.72: *6* tbo.gen_lt
    ...
    #@+node:ekr.20240312053644.74: *7* tbo.is_unary_op
    def is_unary_op(self, i: int, val: str) -> bool:
        ...
        return return_val
    #@+node:ekr.20240312053644.75: *6* tbo.gen_rt
    ...
    #@+node:ekr.20240312053644.76: *6* tbo.gen_star_op
    def gen_star_op(self) -> None:
        ...
        if context == 'arg':
            ...
            self.gen_token('op', val)
            self.gen_blank()
    #@+node:ekr.20240312053644.77: *6* tbo.gen_star_star_op
    ...
    #@+node:ekr.20240312053644.79: *5* tbo.do_string
    ...
    #@+node:ekr.20240312053644.81: *5* tbo.do_ws
    def do_ws(self) -> None:
        """
    ...
    #@+node:ekr.20240312053644.82: *5* tbo.gen_blank
    ...
    #@+node:ekr.20240312053644.83: *5* tbo.gen_token
    def gen_token(self, kind: str, value: Any) -> OutputToken:
        ...
        return tok
    #@+node:ekr.20240312053644.84: *5* tbo.regularize_newlines
    def regularize_newlines(self, s: str) -> str:
        """Regularize newlines within s."""
        return s.replace('\r\n', '\n').replace('\r', '\n')
    #@+node:ekr.20240312053644.85: *4* tbo: Scanning
    # The parser calls scanner methods to move through the list of input tokens.
    #@+node:ekr.20240312053644.86: *5* tbo.pre_scan & helpers
    def pre_scan(self) -> None:
        ...
        for i, token in enumerate(self.tokens):
            ...
            if kind in 'newline':
                ...
                #@+node:ekr.20240312053644.87: *6* << pre-scan 'newline' tokens >>
                ...
                #@+node:ekr.20240312053644.89: *6* << pre-scan 'name' tokens >>
    ...
    def finish_arg(self, end: int, state: ScanState) -> None:
        ...
        for i in range(i1, end):
            ...
            if token.kind not in self.insignificant_kinds:
                ...
                prev = token
    #@+node:ekr.20240312053644.91: *6* tbo.finish_slice
    ...
    #@+node:ekr.20240312053644.95: *5* tbo.is_significant_token
    ...
    #@+node:ekr.20240312053644.96: *5* tbo.next
    ...
    def prev(self, i: int) -> Optional[int]:
        ...
        while i >= 0:
            ...
            i -= 1
        return None  # pragma: no cover
    #@+node:ekr.20240312053644.98: *5* tbo.set_context
