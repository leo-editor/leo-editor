...
class FstringifyTokens (PythonTokenBeautifier):
    ...
    def do_endmarker(self):
        """Handle an endmarker token."""
        pass
    #@+node:ekr.20191024051733.4: *4* fstring.do_errortoken
    def do_errortoken(self):
        """Handle an errortoken token."""
        # This code is executed for versions of Python earlier than 2.4
        if self.val == '@':
            self.op(self.val)
    #@+node:ekr.20191024051733.5: *4* fstring.do_indent & do_dedent (TO DO)
    def do_dedent(self):
        """Handle dedent token."""
        # From self.line_indent
    ...
    #@+node:ekr.20191024051733.8: *4* fstring.do_nl
    def do_nl(self):
        """Handle a continuation line."""
        self.line_end()
    #@+node:ekr.20191024051733.9: *4* fstring.do_number
    def do_number(self):
        """Handle a number token."""
        assert isinstance(self.val, str), repr(self.val)
        self.add_token('number', self.val)
    #@+node:ekr.20191024051733.10: *4* fstring.do_op
    def do_op(self):
        """Handle an op token."""
        val = self.val
        if val == '.':
    ...
    def do_string(self):
        ...
        if self.val.find('\\\n'):
            ...
            # This *does* retain the string's spelling.
        self.blank()
    #@+node:ekr.20191024052024.1: *3* fstring.Output token generators
    #@+node:ekr.20191024052024.2: *4* fstring.add_token
    def add_token(self, kind, value=''):
        """
        Add a token to the code list.
        ...
        tok = self.OutputToken(kind, value)
        self.code_list.append(tok)
    #@+node:ekr.20191024052024.3: *4* fstring.backslash
    def backslash(self):
        """
        Add a backslash token and clear .backslash_seen.
        ...
        self.line_indent()
        self.backslash_seen = False
    #@+node:ekr.20191024052024.4: *4* fstring.blank
    def blank(self):
        """Add a blank request on the code list."""
        prev = self.code_list[-1]
        if prev.kind not in (
            ...
            self.add_token('blank', ' ')
    #@+node:ekr.20191024052024.5: *4* fstring.blank_before_end_line_comment
    def blank_before_end_line_comment(self):
        """Add two blanks before an end-of-line comment."""
        prev = self.code_list[-1]
        self.clean('blank')
    ...
    def blank_lines(self, n):
        ...
        self.add_token('blank-lines', n)
        self.line_indent()
    #@+node:ekr.20191024052024.7: *4* fstring.clean
    def clean(self, kind):
        """Remove the last item of token list if it has the given kind."""
        prev = self.code_list[-1]
        if prev.kind == kind:
            self.code_list.pop()
    #@+node:ekr.20191024052024.8: *4* fstring.clean_blank_lines
    def clean_blank_lines(self):
        """Remove all vestiges of previous lines."""
        table = ('blank-lines', 'line-end', 'line-indent')
        while self.code_list[-1].kind in table:
            self.code_list.pop()
    #@+node:ekr.20191024052024.9: *4* fstring.colon
    def colon(self, val):
        """Handle a colon."""
        if self.square_brackets_level > 0:
            # Put blanks on either side of the colon,
            ...
            self.op_blank(val)
    #@+node:ekr.20191024052024.10: *4* fstring.file_start & file_end
    def file_end(self):
        """
        Add a file-end token to the code list.
        Retain exactly one line-end token.
    ...
    def file_start(self):
        ...
        self.add_token('file-start')
        self.push_state('file-start')
    #@+node:ekr.20191024052024.11: *4* fstring.line_indent
    def line_indent(self, ws=None):
        """Add a line-indent token if indentation is non-empty."""
        self.clean('line-indent')
        ws = ws or self.lws
        if ws:
            self.add_token('line-indent', ws)
    #@+node:ekr.20191024052024.12: *4* fstring.line_end & split/join helpers
    def line_end(self):
        """Add a line-end request to the code list."""
        prev = self.code_list[-1]
        if prev.kind == 'file-start':
        ...
        self.line_indent()
            # Add the indentation for all lines
            # until the next indent or unindent token.
    #@+node:ekr.20191024052024.13: *5* fstring.break_line (new) & helpers
    def break_line(self):
        """
        Break the preceding line, if necessary.
        ...
        self.add_token('line-end', '\n')
        return True
    #@+node:ekr.20191024052024.14: *6* fstring.append_tail
    def append_tail(self, prefix, tail):
        """Append the tail tokens, splitting the line further as necessary."""
        tail_s = ''.join([z.to_string() for z in tail])
        if len(tail_s) < self.max_split_line_length:
        ...
        for i, t in enumerate(tail):
            ...
            if t.kind == 'op' and t.value == ',':
                ...
                self.code_list.append(t)
        g.trace('BAD DELIMS', delim_count)
    #@+node:ekr.20191024052024.15: *6* fstring.find_prev_line (new)
    def find_prev_line(self):
        """Return the previous line, as a list of tokens."""
        line = []
        for t in reversed(self.code_list[: -1]):
            ...
            line.append(t)
        return list(reversed(line))
    #@+node:ekr.20191024052024.16: *6* fstring.find_line_prefix (new)
    def find_line_prefix(self, token_list):
        """
        Return all tokens up to and including the first lt token.
        Also add all lt tokens directly following the first lt token.
        ...
        for i, t in enumerate(token_list):
            ...
            if t.kind == 'lt':
                ...
                break
        return result
    #@+node:ekr.20191024052024.17: *6* fstring.is_any_lt
    def is_any_lt(self, output_token):
        """Return True if the given token is any lt token"""
        return (
            output_token == 'lt'
    ...
    #@+node:ekr.20191024052024.18: *5* fstring.join_lines (new) & helpers
    def join_lines(self):
        """
        Join preceding lines, if the result would be short enough.
        Should be called only at the end of a line.
        ...
        #   Scan back, looking for the first line with all balanced delims.
        #   Do nothing if it is this line.
    #@+node:ekr.20191024052024.19: *4* fstring.lt & rt
    #@+node:ekr.20191024052024.20: *5* fstring.lt
    def lt(self, s):
        """Generate code for a left paren or curly/square bracket."""
        assert s in '([{', repr(s)
        if s == '(':
        ...
        if prev.kind in ('op', 'word-op'):
            ...
            self.op_no_blanks(s)
    #@+node:ekr.20191024052024.21: *5* fstring.rt
    def rt(self, s):
        """Generate code for a right paren or curly/square bracket."""
        assert s in ')]}', repr(s)
        if s == ')':
        ...
        if prev.kind == 'arg-end':
            ...
            self.clean('blank')
        self.add_token('rt', s)
    #@+node:ekr.20191024052024.22: *4* fstring.op*
    def op(self, s):
        """Add op token to code list."""
        assert s and isinstance(s, str), repr(s)
        if self.in_arg_list > 0 and (s in '+-/*' or s == '//'):
    ...
    def op_no_blanks(self, s):
        ...
        self.clean('blank')
        self.add_token('op-no-blanks', s)
    #@+node:ekr.20191024052024.23: *4* fstring.possible_unary_op & unary_op
    def possible_unary_op(self, s):
        """Add a unary or binary op to the token list."""
        self.clean('blank')
        prev = self.code_list[-1]
    ...
    def unary_op(self, s):
        ...
        self.blank()
        self.add_token('unary-op', s)
    #@+node:ekr.20191024052024.24: *4* fstring.star_op (no change)
    def star_op(self):
        """Put a '*' op, with special cases for *args."""
        val = '*'
        if self.paren_level > 0:
            ...
            self.op(val)
    #@+node:ekr.20191024052024.25: *4* fstring.star_star_op (no changed)
    def star_star_op(self):
        """Put a ** operator, with a special case for **kwargs."""
        val = '**'
        if self.paren_level > 0:
            ...
            self.op(val)
    #@+node:ekr.20191024052024.26: *4* fstring.word & word_op
    def word(self, s):
        """Add a word request to the code list."""
        assert s and isinstance(s, str), repr(s)
        if self.in_arg_list > 0:
