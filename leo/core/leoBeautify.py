...
class PythonPrettyPrinter:
    ...
    #@+others
    #@+node:ekr.20150520173107.1: *3* class LeoTidy
    class LeoTidy:
        ...
        #@+others
        #@+node:ekr.20150520173107.2: *4*  f.Entries
        #@+node:ekr.20150520173107.3: *5* f.__call__ (not used)
        # def __call__(self,node):
            # '''__call__ method for AstFormatter class.'''
            # return self.format(node)
        #@+node:ekr.20150520173107.4: *5* f.format
        def format (self,node):
            '''Format the node (or list of nodes) and its descendants.'''
            self.level = 0
            val = self.visit(node)
            return val and val.strip() or ''
        #@+node:ekr.20150520173107.5: *5* f.visit
        def visit(self,node):
            ...
            if isinstance(node,(list,tuple)):
                ...
                return s
        #@+node:ekr.20150520173107.6: *4* f.Contexts
        #@+node:ekr.20150520173107.7: *5* f.ClassDef
        ...
        def do_ClassDef (self,node):
            ...
            return ''.join(result)
        #@+node:ekr.20150520173107.8: *5* f.FunctionDef
        ...
        def do_FunctionDef (self,node):
            ...
            for z in node.body:
                ...
                self.level -= 1
            return ''.join(result)
        #@+node:ekr.20150520173107.9: *5* f.Interactive
        ...
        #@+node:ekr.20150520173107.10: *5* f.Module
        def do_Module (self,node):
            
            # assert 'body' in node._fields
            result = ''.join([self.visit(z) for z in node.body])
            return result
        #@+node:ekr.20150520173107.11: *5* f.Lambda
        def do_Lambda (self,node):
                
            return self.indent('lambda %s: %s\n' % (
                self.visit(node.args),
                self.visit(node.body)))
        #@+node:ekr.20150520173107.12: *4* f.Expressions
        #@+node:ekr.20150520173107.13: *5* f.Expr
        ...
        #@+node:ekr.20150520173107.14: *5* f.Expression
        def do_Expression(self,node):
            
            '''An inner expression: do not indent.'''

            return '%s\n' % self.visit(node.body)
        #@+node:ekr.20150520173107.15: *5* f.GeneratorExp
        ...
        #@+node:ekr.20150520173107.16: *5* f.ctx nodes
        ...
        def do_Store(self,node):
            return 'Store'
        #@+node:ekr.20150520173107.17: *4* f.Operands
        #@+node:ekr.20150520173107.18: *5* f.arguments
        ...
        def do_arguments(self,node):
            ...
            return ','.join(args2)
        #@+node:ekr.20150520173107.19: *5* f.arg (Python3 only)
        # Python 3:
        ...
        def do_arg(self,node):
            ...
            return node.arg
        #@+node:ekr.20150520173107.20: *5* f.Attribute
        ...
        #@+node:ekr.20150520173107.21: *5* f.Bytes
        def do_Bytes(self,node): # Python 3.x only.
            assert g.isPython3
            return str(node.s)
            
        #@+node:ekr.20150520173107.22: *5* f.Call & f.keyword
        ...
        #@+node:ekr.20150520173107.23: *6* f.keyword
        ...
        #@+node:ekr.20150520173107.24: *5* f.comprehension
        def do_comprehension(self,node):
            ...
            return ''.join(result)
        #@+node:ekr.20150520173107.25: *5* f.Dict
        def do_Dict(self,node):
            ...
            return ''.join(result)
        #@+node:ekr.20150520173107.26: *5* f.Ellipsis
        def do_Ellipsis(self,node):
            return '...'

        #@+node:ekr.20150520173107.27: *5* f.ExtSlice
        def do_ExtSlice (self,node):

            return ':'.join([self.visit(z) for z in node.dims])
        #@+node:ekr.20150520173107.28: *5* f.Index
        def do_Index (self,node):
            
            return self.visit(node.value)
        #@+node:ekr.20150520173107.29: *5* f.List
        def do_List(self,node):
            ...
            return '[%s]' % ','.join(elts)
        #@+node:ekr.20150520173107.30: *5* f.ListComp
        ...
        #@+node:ekr.20150520173107.31: *5* f.Name
        def do_Name(self,node):

            return node.id
        #@+node:ekr.20150520182346.1: *5* f.NameConstant
        # Python 3 only.
        ...
        #@+node:ekr.20150520173107.32: *5* f.Num
        def do_Num(self,node):
            return repr(node.n)
        #@+node:ekr.20150520173107.33: *5* f.Repr
        # Python 2.x only
        def do_Repr(self,node):

            return 'repr(%s)' % self.visit(node.value)
        #@+node:ekr.20150520173107.34: *5* f.Slice
        ...
        #@+node:ekr.20150520173107.35: *5* f.Str
        def do_Str (self,node):
            
            '''This represents a string constant.'''
            return repr(node.s)
        #@+node:ekr.20150520173107.36: *5* f.Subscript
        ...
        #@+node:ekr.20150520173107.37: *5* f.Tuple
        def do_Tuple(self,node):
                
            elts = [self.visit(z) for z in node.elts]

            return '(%s)' % ','.join(elts)
        #@+node:ekr.20150520173107.38: *4* f.Operators
        #@+node:ekr.20150520173107.39: *5* f.BinOp
        ...
        #@+node:ekr.20150520173107.40: *5* f.BoolOp
        def do_BoolOp (self,node):
            
            op_name = self.op_name(node.op)
            values = [self.visit(z) for z in node.values]
            return op_name.join(values)
        #@+node:ekr.20150520173107.41: *5* f.Compare
        def do_Compare(self,node):
            ...
            return ''.join(result)
        #@+node:ekr.20150520173107.42: *5* f.UnaryOp
        def do_UnaryOp (self,node):
            
            return '%s%s' % (
                self.op_name(node.op),
                self.visit(node.operand))
        #@+node:ekr.20150520173107.43: *5* f.ifExp (ternary operator)
        ...
        #@+node:ekr.20150520173107.44: *4* f.Statements
        #@+node:ekr.20150520173107.45: *5* f.Assert
        ...
        #@+node:ekr.20150520173107.46: *5* f.Assign
        def do_Assign(self,node):

            return self.indent('%s = %s\n' % (
                '='.join([self.visit(z) for z in node.targets]),
                self.visit(node.value)))
        #@+node:ekr.20150520173107.47: *5* f.AugAssign
        def do_AugAssign(self,node):
            ...
            return self.indent('%s%s=%s\n' % (
                ...
                self.visit(node.value)))
        #@+node:ekr.20150520173107.48: *5* f.Break
        def do_Break(self,node):

            return self.indent('break\n')

        #@+node:ekr.20150520173107.49: *5* f.Continue
        def do_Continue(self,node):

            return self.indent('continue\n')
        #@+node:ekr.20150520173107.50: *5* f.Delete
        def do_Delete(self,node):
            
            targets = [self.visit(z) for z in node.targets]

            return self.indent('del %s\n' % ','.join(targets))
        #@+node:ekr.20150520173107.51: *5* f.ExceptHandler
        def do_ExceptHandler(self,node):
            ...
            for z in node.body:
                ...
                self.level -= 1
            return ''.join(result)
        #@+node:ekr.20150520173107.52: *5* f.Exec
        # Python 2.x only
        ...
        #@+node:ekr.20150520173107.53: *5* f.For
        def do_For (self,node):
            ...
            return ''.join(result)
        #@+node:ekr.20150520173107.54: *5* f.Global
        def do_Global(self,node):

            return self.indent('global %s\n' % (
                ','.join(node.names)))
        #@+node:ekr.20150520173107.55: *5* f.If
        def do_If (self,node):
            ...
            return ''.join(result)
        #@+node:ekr.20150520173107.56: *5* f.Import & helper
        def do_Import(self,node):
            ...
            return self.indent('import %s\n' % (
                ','.join(names)))
        #@+node:ekr.20150520173107.57: *6* f.get_import_names
        def get_import_names (self,node):
            ...
            return result
        #@+node:ekr.20150520173107.58: *5* f.ImportFrom
        def do_ImportFrom(self,node):
            ...
            return self.indent('from %s import %s\n' % (
                node.module,
                ','.join(names)))
        #@+node:ekr.20150520173107.59: *5* f.Pass
        def do_Pass(self,node):

            return self.indent('pass\n')
        #@+node:ekr.20150520173107.60: *5* f.Print (Still a problem)
        # Python 2.x only
        ...
        #@+node:ekr.20150520173107.61: *5* f.Raise
        ...
        #@+node:ekr.20150520173107.62: *5* f.Return
        ...
        #@+node:ekr.20150520173107.63: *5* f.Suite (not used)
        # def do_Suite(self,node):

            # for z in node.body:
                # s = self.visit(z)

        #@+node:ekr.20150520202136.1: *5* f.Try
        ...
        def do_Try(self,node):
            ...
            if node.finalbody:
                ...
                for z in node.finalbody:
                    ...
                    self.level -= 1
            return ''.join(result)
        #@+node:ekr.20150520173107.64: *5* f.TryExcept
        def do_TryExcept(self,node):
            ...
            return ''.join(result)
        #@+node:ekr.20150520173107.65: *5* f.TryFinally
        def do_TryFinally(self,node):
            ...
            return ''.join(result)
        #@+node:ekr.20150520173107.66: *5* f.While
        def do_While (self,node):
            ...
            return ''.join(result)
        #@+node:ekr.20150520173107.67: *5* f.With
        def do_With (self,node):
            ...
            result.append('\n')
            return ''.join(result)
        #@+node:ekr.20150520173107.68: *5* f.Yield
        # Yield(expr? value)
        ...
        def do_Yield(self,node):
            ...
            if hasattr(node,'value'):
                ...
                return 'yield\n'
        #@+node:ekr.20150520173107.69: *4* f.Utils
        #@+node:ekr.20150520173107.70: *5* f.kind
        def kind(self,node):
            '''Return the name of node's class.'''
            return node.__class__.__name__
        #@+node:ekr.20150520173107.71: *5* f.indent
        def indent(self,s):

            return '%s%s' % (' '*4*self.level,s)
        #@+node:ekr.20150520173107.72: *5* f.op_name
