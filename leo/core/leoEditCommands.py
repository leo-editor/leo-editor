...
class abbrevCommandsClass (baseEditCommandsClass):
    ...
    #@+node:ekr.20050920084036.27: *4* expandAbbrev & helpers
    ...
    #@+node:ekr.20131114051702.22732: *5* find_place_holder
...
#@+node:ekr.20060123125256: *3* class minibufferFind(baseEditCommandsClass) (the findHandler)
if not g.new_find:
    class minibufferFind (baseEditCommandsClass):
        ...
        def setupChangePattern (self,pattern):
            ...
            h.update_ivars()
        #@+node:ekr.20060125091234: *5* setupSearchPattern (minibufferFind)
        ...
        def generalSearchHelper (self,pattern,
            ...
            if g.new_find:
                ...
                finder = self.finder
                finder.p = self.c.p
                finder.v = self.finder.p.v
            if findAll:
                finder.findAllCommand()
            elif cloneFindAll:
                finder.cloneFindAllCommand()
            elif cloneFindAllFlattened:
                finder.cloneFindAllFlattenedCommand()
            else:
                # This handles the reverse option.
                finder.findNextCommand()
...
class searchCommandsClass (baseEditCommandsClass):
    ...
    def getPublicCommands (self):
        ...
        if g.new_find:
            ...
            'find-next':                      find.findNextCommand, ### find.findTabFindNext,
            'find-prev':                      find.findPrevCommand, ### find.findTabFindPrev,
            ...
            # 'toggle-find-reverse-option':   find.toggleReverseOption,
    ...
    #@+node:ekr.20060123131421: *4* Top-level methods (**Move to leoFind)
    if not g.new_find:
        #@+others
        ...
        #@-others
    #@+node:ekr.20050920084036.261: *4* incremental search...(**Move to leoFind)
    if not g.new_find:
        #@+others
