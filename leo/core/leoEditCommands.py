...
class EditCommandsManager:
    ...
    #@+others
    #@+node:ekr.20120211121736.10830: *3*  ecm.ctor (defines classesList)
    def __init__ (self,c):
...
class AbbrevCommandsClass (BaseEditCommandsClass):
    ...
    #@+others
    #@+node:ekr.20100901080826.6002: *3* abbrev.Birth
    #@+node:ekr.20100901080826.6003: *4* abbrev.ctor
    def __init__ (self,c):
        ...
        self.w = None
    #@+node:ekr.20150509031550.1: *4* abbrev.cmd (decorator)
    def cmd(name):
    ...
    #@+node:ekr.20130924110246.13738: *5* abbrev.init_abbrev
    ...
    #@+node:ekr.20130924110246.13748: *5* abbrev.init_env
    def init_env(self):
        '''
    ...
    #@+node:ekr.20131113150347.17255: *5* abbrev.init_tree_abbrev
    ...
    #@+node:ekr.20150430051622.1: *3* abbrev.expandAbbrev & helpers (entry point)
    def expandAbbrev (self,event,stroke):
        ...
        return True
    #@+node:ekr.20150430051622.2: *4* abbrev.expand_text
    ...
    #@+node:ekr.20150430051622.3: *4* abbrev.expand_tree & helper
    ...
    #@+node:ekr.20150430051622.4: *4* abbrev.find_place_holder
    def find_place_holder(self,p,do_placeholder):
        ...
        if do_placeholder or c.abbrev_place_start and c.abbrev_place_start in s:
            ...
            return False
    #@+node:ekr.20150430051622.5: *4* abbrev.make_script_substitutions
    ...
    #@+node:ekr.20150430051622.6: *4* abbrev.next_place
    def next_place(self,s,offset=0):
        ...
        return s2,start,end
    #@+node:ekr.20150430051622.7: *4* abbrev.paste_tree
    ...
    #@+node:ekr.20150430051622.8: *4* abbrev.replace_abbrev_name
    ...
    #@+node:ekr.20050920084036.60: *4* abbrev.dynamicCompletion C-M-/
    ...
    def dynamicCompletion (self,event=None):
        ...
        if prefix.strip():
            ...
            c.recolor()
    #@+node:ekr.20050920084036.59: *4* abbrev.dynamicExpansion M-/ & helper
    @cmd('dabbrev-expands')
    ...
    #@+node:ekr.20070605110441: *5* abbrev.dynamicExpandHelper
    def dynamicExpandHelper (self,event,prefix=None,aList=None,w=None):
        ...
        if state == 0:
            ...
            if k.arg:
                ...
                c.recolor()
    #@+node:ekr.20050920084036.61: *4* abbrev.getDynamicList (helper)
    def getDynamicList (self,w,s):
        ...
        return items
    #@+node:ekr.20070531103114: *3* abbrev.static abbrevs
    #@+node:ekr.20100901080826.6001: *4* abbrev.addAbbrevHelper
    ...
    #@+node:ekr.20050920084036.25: *4* abbrev.addAbbreviation
        ...
        if state == 0:
            ...
            if k.arg.strip():
                ...
                k.setLabelGrey(
                    "Abbreviation (on): '%s' = '%s'" % (
                        k.arg,value))
    #@+node:ekr.20051004080550: *4* abbrev.addInverseAbbreviation
    ...
    #@+node:ekr.20050920084036.18: *4* abbrev.killAllAbbrevs
    @cmd('abbrev-kill-all')
    def killAllAbbrevs (self,event):
        ...
        self.abbrevs = {}
    #@+node:ekr.20050920084036.19: *4* abbrev.listAbbrevs
    @cmd('abbrev-list')
    ...
    #@+node:ekr.20050920084036.20: *4* abbrev.readAbbreviations & helper
    @cmd('abbrev-read')
    ...
    #@+node:ekr.20100901080826.6156: *5* abbrev.readAbbreviationsFromFile
    ...
    #@+node:ekr.20050920084036.23: *4* abbrev.toggleAbbrevMode
    ...
    #@+node:ekr.20050920084036.24: *4* abbrev.writeAbbreviation
    @cmd('abbrev-write')
...
class DebugCommandsClass (BaseEditCommandsClass):
    ...
    def __init__ (self,c):
        ...
        BaseEditCommandsClass.__init__(self,c) # init the base class.
    #@+node:ekr.20150509031945.1: *4* db.cmd (decorator)
    def cmd(name):
...
class EditCommandsClass (BaseEditCommandsClass):
    ...
    class To_Python:
        ...
        #@+others
        #@+node:ekr.20150509034116.1: *5* top.cmd (decorator
...
class EditFileCommandsClass (BaseEditCommandsClass):
    ...
    #@+others
    #@+node:ekr.20150509033844.1: *3* ef.Birth
    #@+node:ekr.20050920084036.162: *4* ef.ctor
    def __init__ (self,c):

        BaseEditCommandsClass.__init__(self,c) # init the base class.
    #@+node:ekr.20150509033908.1: *4* ef.cmd (decorator)
    def cmd(name):
...
class KillBufferCommandsClass (BaseEditCommandsClass):
    ...
    def finishCreate (self):
        ...
        BaseEditCommandsClass.finishCreate(self)
            ...
            # This sets self.k
    #@+node:ekr.20150509032244.1: *4* kill.cmd (decorator)
    def cmd(name):
...
class MacroCommandsClass (BaseEditCommandsClass):
    ...
    #@+node:ekr.20150509033720.1: *4* macro.cmd (decorator)
    def cmd(name):
...
class RectangleCommandsClass (BaseEditCommandsClass):
    ...
    #@+others
    #@+node:ekr.20050920084036.222: *3* rectangle.Birth
    ...
    #@+node:ekr.20150509033553.1: *4* rectangle.cmd (decorator)
    def cmd(name):
    ...
    #@+node:ekr.20051215103053: *3* rectangle.beginCommand & beginCommandWithEvent
    def beginCommand (self,undoType='Typing'):
    ...
    def beginCommandWithEvent (self,event,undoType='Typing'):
        ...
        r1,r2,r3,r4 = self.getRectanglePoints(w)
        return w,r1,r2,r3,r4
    #@+node:ekr.20050920084036.224: *3* rectangle.Entries
...
class RegisterCommandsClass (BaseEditCommandsClass):
    ...
    #@+node:ekr.20050920084036.235: *4* register.ctor, init & finishCreate
    def __init__ (self,c):
    ...
    #@+node:ekr.20150509033247.1: *4* register.cmd (decorator)
    def cmd(name):
    ...
    #@+node:ekr.20051004123217: *3* register.checkBodySelection
    def checkBodySelection (self,warning='No text selected'):

        return self._chckSel(event=None,warning=warning)
    #@+node:ekr.20050920084036.236: *3* register.Entries
...
class SpellCommandsClass (BaseEditCommandsClass):
    ...
    #@+node:ekr.20150509032517.1: *4* spell.cmd (decorator)
    def cmd(name):
