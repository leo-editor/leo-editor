...
def os_path_join(*args, **keys):
    ...
    """
    c = keys.get('c')
    uargs = [arg for arg in args if arg]
...
def os_path_normpath(path):
    """Normalize the path."""
    path = os.path.normpath(path)
    ### Not needed. os.path.normpath does this.
        # if g.isWindows:
            # path = path.replace('\\', '/')
    return path
#@+node:ekr.20180314081254.1: *3* g.os_path_normslashes (to be deleted)
