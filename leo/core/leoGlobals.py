...
# @+node:ekr.20260804152751.1: *3* g.add_icon_to_node & helpers
def add_icon_to_node(
    ...
    """Add an icon to p."""
    from PyQt6.QtWidgets import QStyle  # pylint: disable=no-name-in-module
    ...
    ###xoffset = 2
    ###for path in paths:
    ###    xoffset = self.appendImageDictToList(c, aList, path, xoffset)

    d = g.getImageDict(c, icons_dir)
    if not d:
        return
    iconList = g.getIconList(p.v)
    ### aList2.extend(aList)
    iconList.append(d)
    ### self.setIconList(p, iconList)
    g.setIconList(p, iconList)
    p.setDirty()
    c.setChanged()


# @+node:ekr.20260804190852.1: *4* g._hashDict
def _hashDict(d: dict[str, str]) -> str:
    """Hash a dictionary"""
    return ''.join([f"{str(key)}{str(d[key])}" for key in sorted(d)])


# @+node:ekr.20260804190514.2: *4* g._setIconListHelper
def _setIconListHelper(p: Position, iconList: list) -> None:
    """Set icon UA for p.v. to the given list of Icons."""
    v = p.v
    if iconList:  # Update the uA.
        if not hasattr(v, 'unknownAttributes'):
            v.unknownAttributes = {}
        v.unknownAttributes['icons'] = list(iconList)
        v._p_changed = True
    else:  # delete the uA.
...
# @+node:ekr.20260804185834.1: *4* g.getIconList
def getIconList(v: VNode) -> list[dict[str, Any]]:
    """Return list of icons for v."""
    d: list[dict[str, Any]] = []
    if hasattr(v, 'unknownAttributes'):
        d = [dict(i) for i in v.u.get('icons', [])]
        for i in d:
            i['on'] = 'VNode'
    return d


# @+node:ekr.20260804184622.1: *4* g.getImageDict
### def appendImageDictToList(path: str) -> dict:


def getImageDict(c: Cmdr, path: str) -> dict[str, Any]:
    ...
    ### d.update(kargs)  # may switch 'on' to 'VNode'
    return d
...
# @+node:ekr.20260804190514.1: *4* g.setIconList
def setIconList(p: Position, aList: list[Value]) -> None:
    """Set list of icons for position p to aList"""
    current = g.getIconList(p.v)
    if not aList and not current:
        return  # nothing to do
    lHash = ''.join([g._hashDict(i) for i in aList])
    cHash = ''.join([g._hashDict(i) for i in current])
    if lHash == cHash:
        return
    g._setIconListHelper(p, aList)  # set p.u.
