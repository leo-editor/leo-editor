...
class atFile:
    ...
    class compareClass:
        ...
        #@+others
        #@+node:ekr.20100702115533.5827: *3* compare
        def compare (self,fn):
        ...
        #@+node:ekr.20100703060610.5911: *3* compareLines (unified) & helper
        ...
        #@+node:ekr.20100703060610.5913: *4* compareLine
        def compareLine (self,old_i,new_i,old_lines,new_lines):
            ...
            return old_i,new_i
        #@+node:ekr.20100703060610.5914: *4* checkComments
        def checkComments (self,old_i,new_i,old_lines,new_lines):
            ...
            return old_i,new_i
        #@+node:ekr.20100702115533.5828: *3* compareNormalLines
        def compareNormalLines (self,s1,s2):

            cc = self

            aList1 = cc.removeComments(s1)
            aList2 = cc.removeComments(s2)
            n1,n2 = len(aList1),len(aList2)

            for i in range(min(n1,n2)):
                line1,line2 = aList1[i],aList2[i]
                if line1 != line2:
                    print('\nold: %s %s' % (i,repr(line1)))
                    print('new: %s %s' % (i,repr(line2)))
                    return
        #@+node:ekr.20100702115533.5829: *3* compareSentinelLines
        def compareSentinelLines (self,old,new):
            ...
            old_lines = cc.removeNormalLines(old)
            ...
            try:
                ...
                print('old: %s %s' % (old_i,repr(old_s)))
                print('new: %s %s' % (new_i,repr(new_s)))
        #@+node:ekr.20100703060610.5870: *3* ignore
        def ignore (self,fn):
        ...
        #@+node:ekr.20100703060610.5912: *3* helpers
        #@+node:ekr.20100702115533.6254: *4* checkStartNode
        def checkStartNode (self,old_s,new_s):
            ...
            def parse (ver,s,tag):
                at.readVersion5 = ver
                i = at.skipSentinelStart4(s,0)
                assert g.match(s,i,"+node:"),'missing +node: (%s)' % (tag)
                i += 6
                gnx,junk,level,ok = at.parseThinNodeSentinel(s,i)
                if not ok: print('%s_ok: %s' % (tag,s))
                return gnx,ok # old_level is always 0.

            old_gnx,old_ok = parse(False,old_s,'old')
            new_gnx,new_ok = parse(True,new_s,'new')

            assert old_ok and new_ok,'old_ok: %s new_ok: %s' % (old_ok,new_ok)
            assert old_gnx == new_gnx,'old_gnx: %s new_gnx: %s' % (old_gnx,new_gnx)
            # assert old_level == new_level,'level'
        #@+node:ekr.20100702115533.5825: *4* removeComments
        def removeComments(self,source):
            ...
            return aList
        #@+node:ekr.20100702115533.5920: *4* removeNormalLines
        def removeNormalLines(self,source):

            return [s for s in g.splitLines(source)
                        if s.lstrip().startswith('#@')]
        #@+node:ekr.20100703060610.5869: *3* run
        def run (self):
