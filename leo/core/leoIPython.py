...
# Globals...
gipm = None         # The global, singleton GlobalIPythonManager instance.
g_import_ok = False # True if any IPython found.
g_legacy = None     # True if IPython 0.11 or previous found.
g_trace = True      # True: trace imports.
g_root_node = None  # The global root node for some commands.
g_use_ipapp = True
...
#@+others
#@+node:ekr.20120401082519.10036: ** class GlobalIPythonManager & gipm
...
class PosList(list):
    ...
    def select(self, pat):
        ...
        return res
#@+node:ekr.20120401144849.10138: ** Commands
...
def startIPython(event=None):
    ...
    global gipm
    c = event and event.get('c') or gipm

    if not g_import_ok:
        g.es_print('IPython not loaded')
    elif gipm.started:
        gipm.update_commander(c)
    elif g_legacy:
        gipm.start_legacy_api()
    else:
        gipm.start_new_api()
...
def pushToIPythonCommand(event=None):
    ...
    if not gipm.c:
        ...
        if c:
            gipm.c = c
            g.trace('setting gipm.c',c)
        else:
            return g.trace('can not happen: no commander')
   
    g.trace(event)
    g.trace('started',gipm.started)
    g.trace(gipm.c)
    
    if not gipm.started:
        startIPython()
        
    gipm.push_to_ipython()
...
def lee_f(self,s):
    ...
    try:
        if s == 'hist':
            if c:
                wb.ipython_history.b = gipm.get_history()
