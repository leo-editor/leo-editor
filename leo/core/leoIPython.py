...
'''Unified support code for IPython 0.12 and beyond.'''
...
#@+others
#@+node:ekr.20120401063816.10140: ** From ipy_leo.py
#@+node:ekr.20120401063816.10142: *3* Top level
#@+node:ekr.20120401063816.10143: *4*  Init
#@+node:ekr.20120401063816.10144: *5* init_ipython (ipy_leo)
def init_ipython(ipy):
    ...
    wb = LeoWorkbook()
    ip.user_ns['wb'] = wb
#@+node:ekr.20120401063816.10145: *5* update_commander (ipy_leo)
first_launch = True
...
#@+node:ekr.20120401063816.10146: *4* es
...
def es(s):    
    ...
    pass
#@+node:ekr.20120401063816.10147: *4* format_for_leo
@generic
...
def rootnode():
    ...
    _rootnode = None
    return None  
#@+node:ekr.20120401063816.10150: *4* all_cells
def all_cells():
    global _rootnode
    d = {}
    r = rootnode() 
    ...
    for p in nodes:
        ...
        d[h] = p.copy()
    return d    
#@+node:ekr.20120401063816.10151: *4* eval_node
def eval_node(n):
    body = n.b    
...
#@+node:ekr.20120401063816.10152: *4* workbook_complete
...
#@+node:ekr.20120401063816.10153: *4* add_var
def add_var(varname):
    r = rootnode()
    try:
        if r is None:
        ...
        c.redraw()
#@+node:ekr.20120401063816.10154: *4* add_file
def add_file(self,fname):
    p2 = c.currentPosition().insertAfter()
#@+node:ekr.20120401063816.10155: *4* eval_body
def eval_body(body):
    try:
        val = ip.ev(body)
    except:
...
#@+node:ekr.20120401063816.10156: *4* push... (ipy_leo)
#@+node:ekr.20120401063816.10157: *5* expose_ileo_push
def expose_ileo_push(f, prio = 0):

    push_from_leo.add(f, prio)
#@+node:ekr.20120401063816.10158: *5* push_cl_node
...
#@+node:ekr.20120401063816.10159: *5* push_ev_node
...
#@+node:ekr.20120401063816.10160: *5* push_from_leo
push_from_leo = CommandChainDispatcher()
#@+node:ekr.20120401063816.10161: *5* push_ipython_script
def push_ipython_script(node):
    ...
    try:
        ...
        c.redraw()
#@+node:ekr.20120401063816.10162: *5* push_mark_req
def push_mark_req(node):
    ...
    raise TryNext
#@+node:ekr.20120401063816.10163: *5* push_plain_python
...
#@+node:ekr.20120401063816.10164: *5* push_position_from_leo ***
...
#@+node:ekr.20120401063816.10165: *4* edit_object_in_leo
##### @generic
...
#@+node:ekr.20120401063816.10166: *4* edit_macro
...
#@+node:ekr.20120401063816.10167: *4* get_history
def get_history(hstart = 0):
    ...
    for idx in range(hstart, len(ip.IP.input_hist)):
        ...
        if val:
            ...
            res.append('\n')    
    return ''.join(res)
#@+node:ekr.20120401063816.10168: *4* lee_f
def lee_f(self,s):
    ...
    try:
        ...
        c.redraw()
#@+node:ekr.20120401063816.10169: *4* leoref_f
def leoref_f(self,s):
...
#@+node:ekr.20120401063816.10170: *4* mb_f
def mb_f(self, arg):
...
#@+node:ekr.20120401063816.10171: *4* mb_completer
...
#@+node:ekr.20120401063816.10172: *4* ileo_pre_prompt_hook
...
#@+node:ekr.20120401063816.10173: *4* show_welcome
def show_welcome():
    print("------------------")
...
#@+node:ekr.20120401063816.10174: *4* run_leo_startup_node
...
#@+node:ekr.20120401063816.10175: *4* lleo_f
...
#@+node:ekr.20120401063816.10176: *4* lno_f
def lno_f(self, arg):
    """ %lno [note text]
    ...
    child.go()
#@+node:ekr.20120401063816.10177: *4* lshadow_f
def lshadow_f(self, arg):
    ...
    shadow_walk(arg)
    c.redraw()
#@+node:ekr.20120401063816.10178: *4* shadow_walk
...
class PosList(list):
    ...
    def select(self, pat):
        ...
        return res
#@+node:ekr.20120401063816.10181: *3* class LeoWorkbook
class LeoWorkbook:
    ...
    """
    #@+others
    #@+node:ekr.20120401063816.10182: *4* __getattr__
    ...
    #@+node:ekr.20120401063816.10183: *4* __str__
    def __str__(self):
        return "<LeoWorkbook>"

    __repr__ = __str__
    #@+node:ekr.20120401063816.10184: *4* __setattr__
    def __setattr__(self,key, val):
        raise AttributeError(
            "Direct assignment to workbook denied, try wb.%s.v = %s" % (
                key,val))

    #@+node:ekr.20120401063816.10185: *4* __iter__
    def __iter__(self):
        """ Iterate all (even non-exposed) nodes """
        cells = all_cells()
        return (LeoNode(p) for p in c.allNodes_iter())

    #@+node:ekr.20120401063816.10186: *4* current
    current = property(
        lambda self: LeoNode(c.currentPosition()),
        doc = "Currently selected node")
    #@+node:ekr.20120401063816.10187: *4* match_h
    ...
    #@+node:ekr.20120401063816.10188: *4* require
    ...
    #@-others
#@+node:ekr.20120401063816.10189: *3* class LeoNode
class LeoNode(object, UserDict.DictMixin):
    ...
    """
    #@+others
    #@+node:ekr.20120401063816.10190: *4* __init__ (LeoNode)
    def __init__(self,p):
        self.p = p.copy()

    #@+node:ekr.20120401063816.10191: *4* __str__
    def __str__(self):
        return "<LeoNode %s>" % str(self.p.h)

    __repr__ = __str__
    #@+node:ekr.20120401063816.10192: *4* __get_h and _set_h
    def __get_h(self):
    ...
    #@+node:ekr.20120401063816.10193: *4* _get_b and __set_b
    def __get_b(self):
    ...
    def __children(self):
        ...
        for child in self:
            ...
            if not valid_attribute(head):
                ...
                continue
        return d
    #@+node:ekr.20120401063816.10198: *4* keys
    def keys(self):
        d = self.__children()
        return list(d.keys()) # 2010/02/04: per 2to3
    #@+node:ekr.20120401063816.10199: *4* __getitem__
    def __getitem__(self, key):
        ...
        d = self.__children()
        return d[key]
    #@+node:ekr.20120401063816.10200: *4* __setitem__
    ...
    #@+node:ekr.20120401063816.10201: *4* __delitem__
    ...
    #@+node:ekr.20120401063816.10202: *4* ipush (LeoNode)
    def ipush(self):
    ...
    #@+node:ekr.20120401063816.10203: *4* go
    def go(self):
    ...
    #@+node:ekr.20120401063816.10204: *4* append
    def append(self):
        """ Add new node as the last child, return the new node """
        p = self.p.insertAsLastChild()
        return LeoNode(p)
    #@+node:ekr.20120401063816.10205: *4* script
    def script(self):
    ...
    #@+node:ekr.20120401063816.10206: *4* __get_uA
    def __get_uA(self):
        p = self.p
...
#@+node:ekr.20120401063816.10228: ** From ipython.py
#@+node:ekr.20120401063816.10233: *3* Module-level functions from ipython.py (not used)
if 0:
    #@+others
    #@+node:ekr.20120401063816.10234: *4* init
    def init ():
        ...
        return True
    #@+node:ekr.20120401063816.10235: *4* onCreate
    ...
    #@-others
#@+node:ekr.20120401063816.10236: *3* class ipythonController
class ipythonController:
    ...
    #@+others
    #@+node:ekr.20120401063816.10237: *4* Birth
    #@+node:ekr.20120401063816.10238: *5* ctor
    def __init__ (self,c):

        self.c = c
        self.createCommands()
    #@+node:ekr.20120401063816.10239: *5* createCommands
    ...
    #@+node:ekr.20120401063816.10240: *4* Commands
    #@+node:ekr.20120401063816.10241: *5* startIPython (not used)
    def startIPython(self,event=None):
        ...
        if 0: ### Old code
            ...
            try:
                ...
                g.es_exception()
    #@+node:ekr.20120401063816.10242: *5* pushToIPythonCommand
    ...
    #@+node:ekr.20120401063816.10243: *4* Utils...
    #@+node:ekr.20120401063816.10244: *5* error & message
    def error (self,s):
    ...
    #@+node:ekr.20120401063816.10245: *5* pushToIPython
    ...
    #@+node:ekr.20120401063816.10246: *5* started
    def started (self):
        
        ### return gIP
        return ishell.InteractiveShell.instance()
    #@-others
#@+node:ekr.20120401063816.10247: *3* class leoInterface
class leoInterface:
