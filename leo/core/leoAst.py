...
class Test:
    ...
    #@+others
    #@+node:ekr.20150521120511.1: *3* test.aDecorator
    def aDecorator(func):
        '''A do-nothing decorator. Required so this file loads.'''
        # pylint: disable=no-self-argument
        return func
    #@+node:ekr.20150521052355.6: *3* test.Contexts
    #@+node:ekr.20150521052355.7: *4* test.ClassDef
    # ClassDef(identifier name, expr* bases, stmt* body, expr* decorator_list)
    ...
    class aClass4(aClass2):
        def eggs(self):
            self.spam()
    #@+node:ekr.20150521052355.8: *4* test.FunctionDef
    # FunctionDef(identifier name, arguments args, stmt* body, expr* decorator_list)
    ...
    def aFunc3(*args,**keys):
        aFunc2(*args,**keys)
        
    def aFunc4(arg1,arg2=None,arg3='abc'):
        afunc3(arg2)
        
    def aFunc5(arg1,arg2,arg3={},arg4=[],*args,**keys):
        afunc4(arg2=arg1,arg3=args)
        
    #@+node:ekr.20150521052355.11: *4* test.Lambda
    def aLambda0():
    ...
    def aLambda2():
        ...
        f = lambda a,b: a+b
        print(f(1,2))
    #@+node:ekr.20150521052355.17: *3* test.Operands (to do)
    #@+node:ekr.20150521052355.20: *4* test.Attribute
    # Attribute(expr value, identifier attr, expr_context ctx)
    ...
    def Attribute(self,node):
        ...
        return '%s.%s' % (
            self.visit(node.value),
            node.attr) # Don't visit node.attr: it is always a string.
    #@+node:ekr.20150521052355.21: *4* test.Bytes
    def aBytes(): # Python 3.x only.

        print(bytes('abc'))
    #@+node:ekr.20150521052355.22: *4* test.Call & test.keyword
    # Call(expr func, expr* args, keyword* keywords, expr? starargs, expr? kwargs)
    ...
    def aCall():
        ...
        c = f(a=2)
        d = f(*args,**keys)
    #@+node:ekr.20150521052355.23: *5* test.keyword
    # keyword = (identifier arg, expr value)
    ...
    def keyword(self,node):
        ...
        # This is a keyword *arg*, not a Python keyword!
        return '%s=%s' % (node.arg,value)
    #@+node:ekr.20150521052355.24: *4* test.comprehension
    def comprehension(self,node):
        ...
        return ''.join(result)
    #@+node:ekr.20150521052355.25: *4* test.Dict
    def Dict(self,node):
        ...
        return ''.join(result)
    #@+node:ekr.20150521052355.26: *4* test.Ellipsis
    def Ellipsis(self,node):
        return '...'

    #@+node:ekr.20150521052355.27: *4* test.ExtSlice
    def ExtSlice (self,node):

        return ':'.join([self.visit(z) for z in node.dims])
    #@+node:ekr.20150521052355.15: *4* test.GeneratorExp (to do)
    def aGeneratorExp():
        
        return [z for z in aList]
       
    #@+node:ekr.20150521052355.28: *4* test.Index
    def Index (self,node):
        
        return self.visit(node.value)
    #@+node:ekr.20150521052355.29: *4* test.List
    def List(self,node):
        ...
        elst = [z for z in elts if z] # Defensive.
        return '[%s]' % ','.join(elts)
    #@+node:ekr.20150521052355.30: *4* test.ListComp
    def ListComp(self,node):
        ...
        return '%s for %s' % (elt,''.join(gens))
    #@+node:ekr.20150521052355.31: *4* test.Name
    def Name(self,node):

        return node.id
    #@+node:ekr.20150521052355.32: *4* test.Num
    def Num(self,node):
        return repr(node.n)
    #@+node:ekr.20150521052355.33: *4* test.Repr
    # Python 2.x only
    def Repr(self,node):

        return 'repr(%s)' % self.visit(node.value)
    #@+node:ekr.20150521052355.34: *4* test.Slice
    def Slice (self,node):
        ...
        if step:
            ...
            return '%s:%s' % (lower,upper)
    #@+node:ekr.20150521052355.35: *4* test.Str
    def Str (self,node):
        
        '''This represents a string constant.'''
        return repr(node.s)
    #@+node:ekr.20150521052355.36: *4* test.Subscript
    # Subscript(expr value, slice slice, expr_context ctx)
    ...
    def Subscript(self,node):
        ...
        return '%s[%s]' % (value,the_slice)
    #@+node:ekr.20150521052355.37: *4* test.Tuple
    def Tuple(self,node):
            
        elts = [self.visit(z) for z in node.elts]

        return '(%s)' % ','.join(elts)
    #@+node:ekr.20150521052355.38: *3* test.Operators (to do)
    #@+node:ekr.20150521052355.39: *4* test.BinOp
    def BinOp (self,node):
    ...
    #@+node:ekr.20150521052355.40: *4* test.BoolOp
    ...
    #@+node:ekr.20150521052355.41: *4* test.Compare
    def Compare(self,node):
        ...
        return ''.join(result)
    #@+node:ekr.20150521052355.42: *4* test.UnaryOp
    def UnaryOp (self,node):
        
        return '%s%s' % (
            self.op_name(node.op),
            self.visit(node.operand))
    #@+node:ekr.20150521052355.43: *4* test.ifExp (ternary operator)
    def IfExp (self,node):
        ...
        return '%s if %s else %s ' % (
            ...
            self.visit(node.test),
            self.visit(node.orelse))
    #@+node:ekr.20150521052355.44: *3* test.Statements (to do)
    #@+node:ekr.20150521052355.45: *4* test.Assert
    def Assert(self,node):
        ...
        if getattr(node,'msg',None):
            ...
            return self.indent('assert %s' % test)
    #@+node:ekr.20150521052355.46: *4* test.Assign
    def Assign(self,node):

        return self.indent('%s=%s' % (
            '='.join([self.visit(z) for z in node.targets]),
            self.visit(node.value)))
    #@+node:ekr.20150521052355.47: *4* test.AugAssign
    def AugAssign(self,node):
        ...
        return self.indent('%s%s=%s\n' % (
            ...
            self.op_name(node.op), # Bug fix: 2013/03/08.
            self.visit(node.value)))
    #@+node:ekr.20150521052355.48: *4* test.Break
    def Break(self,node):

        return self.indent('break\n')

    #@+node:ekr.20150521052355.49: *4* test.Continue
    def Continue(self,node):

        return self.indent('continue\n')
    #@+node:ekr.20150521052355.50: *4* test.Delete
    def Delete(self,node):
        
        targets = [self.visit(z) for z in node.targets]

        return self.indent('del %s\n' % ','.join(targets))
    #@+node:ekr.20150521052355.51: *4* test.ExceptHandler
    def ExceptHandler(self,node):
        ...
        for z in node.body:
            ...
            self.level -= 1
        return ''.join(result)
    #@+node:ekr.20150521052355.52: *4* test.Exec
    # Python 2.x only
    def Exec(self,node):
        ...
        if args:
            ...
            return self.indent('exec %s\n' % (body))
    #@+node:ekr.20150521052355.53: *4* test.For
    def aFor (self,node):
        ...
        return ''.join(result)
    #@+node:ekr.20150521052355.54: *4* test.Global
    def Global(self,node):

        return self.indent('global %s\n' % (
            ','.join(node.names)))
    #@+node:ekr.20150521052355.55: *4* test.If
    def If (self,node):
        ...
        return ''.join(result)
    #@+node:ekr.20150521052355.56: *4* test.Import
    def Import(self,node):
        ...
        return self.indent('import %s\n' % (
            ','.join(names)))
    #@+node:ekr.20150521052355.58: *4* test.ImportFrom
    def ImportFrom(self,node):
        ...
        return self.indent('from %s import %s\n' % (
            node.module,
            ','.join(names)))
    #@+node:ekr.20150521052355.59: *4* test.Pass
    def Pass(self,node):

        return self.indent('pass\n')
    #@+node:ekr.20150521052355.60: *4* test.Print
    # Python 2.x only
    def Print(self,node):
        ...
        return self.indent('print(%s)\n' % (
            ','.join(vals)))
    #@+node:ekr.20150521052355.61: *4* test.Raise
    def Raise(self,node):
        ...
        if args:
            ...
            return self.indent('raise\n')
    #@+node:ekr.20150521052355.62: *4* test.Return
    def Return(self,node):
        ...
        if node.value:
            ...
            return self.indent('return\n')
    #@+node:ekr.20150521052355.63: *4* test.Suite
    # def Suite(self,node):

        # for z in node.body:
            # s = self.visit(z)

    #@+node:ekr.20150521052355.64: *4* test.TryExcept
    def TryExcept(self,node):
        ...
        return ''.join(result)
    #@+node:ekr.20150521052355.65: *4* test.TryFinally
    def TryFinally(self,node):
        ...
        return ''.join(result)
    #@+node:ekr.20150521052355.66: *4* test.While
    def While (self,node):
        ...
        return ''.join(result)
    #@+node:ekr.20150521052355.67: *4* test.With
    def With (self,node):
        ...
        result.append('\n')
        return ''.join(result)
    #@+node:ekr.20150521052355.68: *4* test.Yield
    def Yield(self,node):
