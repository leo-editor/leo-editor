...
if 1:  # pragma: no cover
    ...
    #@+node:ekr.20240105051733.1: *4* function: make_tokens (for unit tests)
    def make_tokens(contents: str) -> list[Token]:
        ...
        """
        return Tokenizer().make_tokens(contents)
...
class AstDumper:  # pragma: no cover
    ...
    #@-others
#@+node:ekr.20240105041303.1: *3* class BaseTokenizer
class BaseTokenizer:
    """
    The base class for tokenizers that create Tokens or InputTokens.
    """
    results: list[AnyToken] = []

    #@+others
    #@+node:ekr.20240105041147.4: *4* base_tok.check_results
    ...
    #@+node:ekr.20240105043451.1: *4* base_tok.check_round_trip
    def check_round_trip(self, contents: str, tokens: list[AnyToken]) -> bool:
        ...
        if not ok:
            ...
            g.printObj(result)
        return ok
    #@+node:ekr.20191110165235.3: *4* base_tok.create_tokens
    def create_tokens(self, contents: str, five_tuples: Generator) -> list[AnyToken]:
        """
        ...
        return self.results
    #@+node:ekr.20191110165235.4: *4* base_tok.do_token (the gem)
    ...
    #@+node:ekr.20240105043300.1: *4* base_tok.make_tokens
    def make_tokens(self, contents: str) -> list[AnyToken]:
        """
        Return a list (not a generator) of Token objects corresponding to the
        list of 5-tuples generated by tokenize.tokenize.
        ...
        return tokens
    #@+node:ekr.20240105043550.1: *4* base_tok.tokens_to_string
    def tokens_to_string(self, tokens: list[AnyToken]) -> str:
        """Return the string represented by the list of tokens."""
        if tokens is None:
            # This indicates an internal error.
            print('')
...
class Orange:
    ...
    def init_tokens_from_file(self, filename: str) -> tuple[str, str, list[InputToken]]:  # pragma: no cover  list[InputToken]
        """
        ...
        self.tokens = tokens = InputTokenizer().make_tokens(contents)
        return contents, encoding, tokens
...
class Token:
    ...
    def __init__(self, kind: str, value: str):
        ...
        self.line = ''  # The entire line containing the token.
        self.line_number = 0  # The line number, for errors and dumps.
        self.node: Optional[Node] = None  # The Ast node corresponding to this token.
    ...
    def show_val(self, truncate_n: int) -> str:  # pragma: no cover
        ...
        return val
    #@-others
#@+node:ekr.20191110165235.1: *3* class Tokenizer (BaseTokenizer)
class Tokenizer(BaseTokenizer):
    ...
    #@+others
    #@+node:ekr.20191110165235.2: *4* tok.add_token
    token_index = 0
...
class InputTokenizer(BaseTokenizer):
    ...
    #@+others
    #@+node:ekr.20240105041147.2: *4* itok.add_token
    token_index = 0
    prev_line_token = None
...
if TYPE_CHECKING:
    # from leo.core.leoAst import Token, InputToken
    AnyToken: TypeAlias = Union[Token, InputToken]
