...
from docutils.utils.u import u,ur
### from docutils.utils.u import ua as u ### works, except for one instance, corrected below.
### from docutils.utils.u import u ### many errors
...
class Babel(object):
    ...
    def __init__(self, language_code, reporter=None):
        ...
        if self.language in ('ngerman', 'german', 'austrian', 'naustrian',
            ...
            self.literal_double_quote = ur(r'\dq{}')
        if self.language == 'french':
            self.quotes = (r'\og{}', r'\fg{}')
        if self.language == 'italian':
            self.literal_double_quote = ur(r'{\char`\"}')
...
class CharMaps(object):
    ...
    special = {
        ...
        ord('%'): ur(r'\%'),
        ord('&'): ur(r'\&'),
        ord('~'): ur(r'\textasciitilde{}'),
        ord('_'): ur(r'\_'),
        ord('^'): ur(r'\textasciicircum{}'),
        ord('\\'): ur(r'\textbackslash{}'),
        ord('{'): ur(r'\{'),
        ord('}'): ur(r'\}'),
        ...
        0x00AD: ur(r'\-'), # SOFT HYPHEN
    }
    # Unicode chars that are not recognized by LaTeX's utf8 encoding
    unsupported_unicode = {
        0x00A0: ur(r'~'), # NO-BREAK SPACE
        # TODO: ensure white space also at the beginning of a line?
        # 0x00A0: ur'\leavevmode\nobreak\vadjust{}~'
        0x2008: ur(r'\,'), # PUNCTUATION SPACE   
        0x2011: ur(r'\hbox{-}'), # NON-BREAKING HYPHEN
        0x202F: ur(r'\,'), # NARROW NO-BREAK SPACE
        0x21d4: ur(r'$\Leftrightarrow$'),
        # Docutils footnote symbols:
        0x2660: ur(r'$\spadesuit$'),
        0x2663: ur(r'$\clubsuit$'),
    }
    # Unicode chars that are recognized by LaTeX's utf8 encoding
    utf8_supported_unicode = {
        0x00AB: ur(r'\guillemotleft'), # LEFT-POINTING DOUBLE ANGLE QUOTATION MARK
        0x00bb: ur(r'\guillemotright'), # RIGHT-POINTING DOUBLE ANGLE QUOTATION MARK
        0x200C: ur(r'\textcompwordmark'), # ZERO WIDTH NON-JOINER
        0x2013: ur(r'\textendash{}'),
        0x2014: ur(r'\textemdash{}'),
        0x2018: ur(r'\textquoteleft{}'),
        0x2019: ur(r'\textquoteright{}'),
        0x201A: ur(r'\quotesinglbase{}'), # SINGLE LOW-9 QUOTATION MARK
        0x201C: ur(r'\textquotedblleft{}'),
        0x201D: ur(r'\textquotedblright{}'),
        0x201E: ur(r'\quotedblbase{}'), # DOUBLE LOW-9 QUOTATION MARK
        0x2030: ur(r'\textperthousand{}'),   # PER MILLE SIGN
        0x2031: ur(r'\textpertenthousand{}'), # PER TEN THOUSAND SIGN
        0x2039: ur(r'\guilsinglleft{}'),
        0x203A: ur(r'\guilsinglright{}'),
        0x2423: ur(r'\textvisiblespace{}'),  # OPEN BOX
        0x2020: ur(r'\dag{}'),
        0x2021: ur(r'\ddag{}'),
        0x2026: ur(r'\dots{}'),
        0x2122: ur(r'\texttrademark{}'),
    }
    # recognized with 'utf8', if textcomp is loaded
    textcomp = {
        # Latin-1 Supplement
        0x00a2: ur(r'\textcent{}'),          # ¢ CENT SIGN
        0x00a4: ur(r'\textcurrency{}'),      # ¤ CURRENCY SYMBOL
        0x00a5: ur(r'\textyen{}'),           # ¥ YEN SIGN
        0x00a6: ur(r'\textbrokenbar{}'),     # ¦ BROKEN BAR
        0x00a7: ur(r'\textsection{}'),       # § SECTION SIGN
        0x00a8: ur(r'\textasciidieresis{}'), # ¨ DIAERESIS
        0x00a9: ur(r'\textcopyright{}'),     # © COPYRIGHT SIGN
        0x00aa: ur(r'\textordfeminine{}'),   # ª FEMININE ORDINAL INDICATOR
        0x00ac: ur(r'\textlnot{}'),          # ¬ NOT SIGN
        0x00ae: ur(r'\textregistered{}'),    # ® REGISTERED SIGN
        0x00af: ur(r'\textasciimacron{}'),   # ¯ MACRON
        0x00b0: ur(r'\textdegree{}'),        # ° DEGREE SIGN
        0x00b1: ur(r'\textpm{}'),            # ± PLUS-MINUS SIGN
        0x00b2: ur(r'\texttwosuperior{}'),   # ² SUPERSCRIPT TWO
        0x00b3: ur(r'\textthreesuperior{}'), # ³ SUPERSCRIPT THREE
        0x00b4: ur(r'\textasciiacute{}'),    # ´ ACUTE ACCENT
        0x00b5: ur(r'\textmu{}'),            # µ MICRO SIGN
        0x00b6: ur(r'\textparagraph{}'),     # ¶ PILCROW SIGN # not equal to \textpilcrow
        0x00b9: ur(r'\textonesuperior{}'),   # ¹ SUPERSCRIPT ONE
        0x00ba: ur(r'\textordmasculine{}'),  # º MASCULINE ORDINAL INDICATOR
        0x00bc: ur(r'\textonequarter{}'),    # 1/4 FRACTION
        0x00bd: ur(r'\textonehalf{}'),       # 1/2 FRACTION
        0x00be: ur(r'\textthreequarters{}'), # 3/4 FRACTION
        0x00d7: ur(r'\texttimes{}'),         # × MULTIPLICATION SIGN
        0x00f7: ur(r'\textdiv{}'),           # ÷ DIVISION SIGN
        #
        0x0192: ur(r'\textflorin{}'),        # LATIN SMALL LETTER F WITH HOOK
        0x02b9: ur(r'\textasciiacute{}'),    # MODIFIER LETTER PRIME
        0x02ba: ur(r'\textacutedbl{}'),      # MODIFIER LETTER DOUBLE PRIME
        0x2016: ur(r'\textbardbl{}'),        # DOUBLE VERTICAL LINE
        0x2022: ur(r'\textbullet{}'),        # BULLET
        0x2032: ur(r'\textasciiacute{}'),    # PRIME
        0x2033: ur(r'\textacutedbl{}'),      # DOUBLE PRIME
        0x2035: ur(r'\textasciigrave{}'),    # REVERSED PRIME
        0x2036: ur(r'\textgravedbl{}'),      # REVERSED DOUBLE PRIME
        0x203b: ur(r'\textreferencemark{}'), # REFERENCE MARK
        0x203d: ur(r'\textinterrobang{}'),   # INTERROBANG
        0x2044: ur(r'\textfractionsolidus{}'), # FRACTION SLASH
        0x2045: ur(r'\textlquill{}'),        # LEFT SQUARE BRACKET WITH QUILL
        0x2046: ur(r'\textrquill{}'),        # RIGHT SQUARE BRACKET WITH QUILL
        0x2052: ur(r'\textdiscount{}'),      # COMMERCIAL MINUS SIGN
        0x20a1: ur(r'\textcolonmonetary{}'), # COLON SIGN
        0x20a3: ur(r'\textfrenchfranc{}'),   # FRENCH FRANC SIGN
        0x20a4: ur(r'\textlira{}'),          # LIRA SIGN
        0x20a6: ur(r'\textnaira{}'),         # NAIRA SIGN
        0x20a9: ur(r'\textwon{}'),           # WON SIGN
        0x20ab: ur(r'\textdong{}'),          # DONG SIGN
        0x20ac: ur(r'\texteuro{}'),          # EURO SIGN
        0x20b1: ur(r'\textpeso{}'),          # PESO SIGN
        0x20b2: ur(r'\textguarani{}'),       # GUARANI SIGN
        0x2103: ur(r'\textcelsius{}'),       # DEGREE CELSIUS
        0x2116: ur(r'\textnumero{}'),        # NUMERO SIGN
        0x2117: ur(r'\textcircledP{}'),      # SOUND RECORDING COYRIGHT
        0x211e: ur(r'\textrecipe{}'),        # PRESCRIPTION TAKE
        0x2120: ur(r'\textservicemark{}'),   # SERVICE MARK
        0x2122: ur(r'\texttrademark{}'),     # TRADE MARK SIGN
        0x2126: ur(r'\textohm{}'),           # OHM SIGN
        0x2127: ur(r'\textmho{}'),           # INVERTED OHM SIGN
        0x212e: ur(r'\textestimated{}'),     # ESTIMATED SYMBOL
        0x2190: ur(r'\textleftarrow{}'),     # LEFTWARDS ARROW
        0x2191: ur(r'\textuparrow{}'),       # UPWARDS ARROW
        0x2192: ur(r'\textrightarrow{}'),    # RIGHTWARDS ARROW
        0x2193: ur(r'\textdownarrow{}'),     # DOWNWARDS ARROW
        0x2212: ur(r'\textminus{}'),         # MINUS SIGN
        0x2217: ur(r'\textasteriskcentered{}'), # ASTERISK OPERATOR
        0x221a: ur(r'\textsurd{}'),          # SQUARE ROOT
        0x2422: ur(r'\textblank{}'),         # BLANK SYMBOL
        0x25e6: ur(r'\textopenbullet{}'),    # WHITE BULLET
        0x25ef: ur(r'\textbigcircle{}'),     # LARGE CIRCLE
        0x266a: ur(r'\textmusicalnote{}'),   # EIGHTH NOTE
        0x26ad: ur(r'\textmarried{}'),       # MARRIAGE SYMBOL
        0x26ae: ur(r'\textdivorced{}'),      # DIVORCE SYMBOL
        0x27e8: ur(r'\textlangle{}'),        # MATHEMATICAL LEFT ANGLE BRACKET
        0x27e9: ur(r'\textrangle{}'),        # MATHEMATICAL RIGHT ANGLE BRACKET
    }
    # Unicode chars that require a feature/package to render
    pifont = {
        0x2665: ur(r'\ding{170}'),     # black heartsuit
        0x2666: ur(r'\ding{169}'),     # black diamondsuit
        0x2713: ur(r'\ding{51}'),      # check mark
        0x2717: ur(r'\ding{55}'),      # check mark
    }
    # TODO: greek alphabet ... ?
...
class LaTeXTranslator(nodes.NodeVisitor):
    ...
    attribution_formats = {'dash': (u'—', ''), # EM DASH ###### only change.
                           'parentheses': ('(', ')'),
                           'parens': ('(', ')'),
                           'none': ('', '')}
